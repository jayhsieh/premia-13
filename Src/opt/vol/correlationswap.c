...
static NumFunc_1 correlationswap=
  {
    Call,
...
static TYPEOPT CorrelationSwap=
  {
  ...
  if (opt->init == 0 )
    {
      ...
      pt->PayOff.Val.V_NUMFUNC_1=&correlationswap;

      (pt->EuOrAm).Val.V_BOOL=EURO;
      (pt->Maturity).Val.V_DATE=1.0;
            (pt->VarianceBudget).Val.V_PDOUBLE=0.0625;

      (pt->PayOff.Val.V_NUMFUNC_1)->Par[0].Val.V_PDOUBLE=0.5;
    }

  return OK;
}

MAKEOPT(CorrelationSwap);
