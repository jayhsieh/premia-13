...
static NumFunc_nd callbasketeuro_nd=
{
  CallBasket_nd,
...
static TYPEOPT CallBasketEuro_nd=
{
...
static int OPT(Init)(Option *opt,Model *mod)
{
  ...
  if ( opt->init == 0)
    {
      opt->init = 1;
      opt->HelpFilenameHint = "callbasketeuro";
      opt->nvar = 4;
      opt->nvar_setable=2;

      pt->PayOff.Val.V_NUMFUNC_ND=&callbasketeuro_nd;
