...
static NumFunc_nd putgeomamer_nd=
{
  PutGeom_nd,
...
static int OPT(Init)(Option *opt,Model *mod)
{
  ...
  if ( opt->init == 0)
    {
      opt->init = 1;
       opt->HelpFilenameHint = "putgeomamer";
      opt->nvar = 4;
      opt->nvar_setable=2;

      pt->PayOff.Val.V_NUMFUNC_ND=&putgeomamer_nd;
