...
static NumFunc_nd putmultispreadeuro_nd=
{
  PutMultiSpread_nd,
...
static TYPEOPT PutMultiSpreadEuro_nd=
{
...
static int OPT(Init)(Option *opt,Model *mod)
{
  ...
  if ( opt->init == 0)
    {
      opt->init = 1;
      opt->HelpFilenameHint = "putmultispreadeuro";
      opt->nvar = 4;
      opt->nvar_setable=2;

      pt->PayOff.Val.V_NUMFUNC_ND=&putmultispreadeuro_nd;
