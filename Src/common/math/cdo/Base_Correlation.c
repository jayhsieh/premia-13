...
/**intensité implicite déduite des données du marché sur le CDS on utilise la
   méthode de Newton pour pouvoir calculer le zero contrat CDS */
...
/**On suppose qu'on a les données sur les 5 tranches + l'index on cherche à
   determiner la correlation implicite sur  chaque tranche par inversion de la
   formule su spread **/
...
double prix_contrat_CDO(const prod * produit,const double s,const double rho,const double lambda){
  ...
  double fc=pnl_normal_density(centre)*eval_contrat(produit,s1,rho,lambda,centre);
  resk=wgk[11]*fc;
  for(j=1;j<=10;j++){
    absc=h*xgk[j];
    fval1=pnl_normal_density(centre-absc)*eval_contrat(produit,s1,rho,lambda,centre-absc);
    fval2=pnl_normal_density(centre+absc)*eval_contrat(produit,s1,rho,lambda,centre+absc);
    fsum=fval1+fval2;
...
double loss(const prod * produit,const double rho,const double lambda)
{
  ...
  centre=(a+b)*0.5;
  h=(b-a)*0.5;
  fc=pnl_normal_density(centre)*eval_loss(produit,rho,lambda,centre);
  resk=wgk[11]*fc;
  for(j=1;j<=10;j++){
    absc=h*xgk[j];
    fval1=pnl_normal_density(centre-absc)*eval_loss(produit,rho,lambda,centre-absc);
    fval2=pnl_normal_density(centre+absc)*eval_loss(produit,rho,lambda,centre+absc);
    fsum=fval1+fval2;
    resk=resk+wgk[j]*fsum;
  }
  return resk*h;
...
double payment_leg_CDO(const prod * produit,const double rho,const double lambda){
  ...
  double centre=(a+b)*0.5;
  double h=(b-a)*0.5;
  double fc=pnl_normal_density(centre)*payleg_cond_CDO(produit,rho,lambda,centre);
  int j;
  resk=wgk[11]*fc;
  for(j=1;j<=10;j++){
    absc=h*xgk[j];
    fval1=pnl_normal_density(centre-absc)*payleg_cond_CDO(produit,rho,lambda,centre-absc);
    fval2=pnl_normal_density(centre+absc)*payleg_cond_CDO(produit,rho,lambda,centre+absc);
    fsum=fval1+fval2;
    resk=resk+wgk[j]*fsum;
  }
  return resk*h;
...
double spread_CDO(const prod * produit,const double rho,const double lambda){
  ...
  return (100*(loss(produit,rho,lambda)-0.05*(payment_leg_CDO( produit,rho,lambda))))/(b*K*M);
                      ...
                      const int choix )
{
  ...
  if(choix==1){
    ...
    /** On connait la base-correl de la tranche [0,0.03],on déduira ainsi les
