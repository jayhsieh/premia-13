...
void CIR_diffusion_update(CIR_diffusion *  process,double t)
{
  if(process->time!=t)
    {
...
dcomplex GammaOU_diffusion_characteristic_exponent_without_cast(dcomplex u,double t,GammaOU_diffusion * mod)
{
  ...
  dcomplex miu,i_psi_u;
  GammaOU_diffusion_update(mod,t);
  miu=Complex(u.i,-u.r);
  i_psi_u=mod->characteristic_exponent(u,mod->Levy);
  i_psi_u=Complex(i_psi_u.i,-i_psi_u.r);
  return
...
void GammaOU_diffusion_update(GammaOU_diffusion * process,double t)
{
  if(process->time!=t)
    {
      dcomplex i_psi_u;
                                           ...
                                           double *jump_drift)
{
  ...
  process->Drift=0;
  printf(" eta= %7.4f kappa = %7.4f rho = %7.4f theta =  %7.4f sigma_0 =  %7.4f \n", process->Eta,process->Kappa,process->Rho,process->Theta,
         process->Sigma);
                                         ...
                                         double Sigma_J_,double Lambda_J_,double *jump_drift)
{
  ...
  //  (*jump_drift)= -process->C_Gamma_minus_Alpha_Minus*process->Lambdap1powAlphaMinus;
  //process->Lambdap1powAlphaMinus=0.0;
  printf(" eta= %7.4f kappa = %7.4f rho = %7.4f theta =  %7.4f sigma_0 =  %7.4f , mu_J = %7.4f , Sigma_J= %7.4f , Lambda_J = %7.4f\n",
         process->Eta,process->Kappa,process->Rho,process->Theta,
         process->Sigma,process->mu_J,process->Sigma_J,process->Lambda_J);
  return process;
}
                                     ...
                                     double *jump_drift)
{
  ...
  printf(" eta= %7.4f kappa = %7.4f rho = %7.4f theta =  %7.4f sigma_0 =  %7.4f , mu_J = %7.4f , Sigma_J= %7.4f , Lambda_J = %7.4f\n",
         process->Eta,process->Kappa,process->Rho,process->Theta,
         process->Sigma,process->mu_y,process->Sigma_y_sqr_demi,process->Lambda_y);
  return process;
}
...
dcomplex Levy_diffusion_ln_characteristic_function(dcomplex u,double t,void * mod)
{
  Levy_diffusion *ptmod = (Levy_diffusion *) mod;
  ...
  dcomplex phi=ptmod->ln_characteristic_function(u,t,ptmod->process);
...
Levy_diffusion * Levy_diffusion_create(void * process_, dcomplex (*characretristic_exponent_)(dcomplex u,double t,void * mod),
