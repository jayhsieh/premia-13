...
double cf_hw1d_zbput(ZCMarketData* ZCMarket, double a,double sigma, double S, double T, double K)
{
    ...
    /*Price*/
    price = P0S * (cdf_nor(d1)-1) - K * P0T * (cdf_nor(d2)-1);

    return price;
}

/*Put Option*/
static int cf_zbp1d(double flat_flag, double a,double sigma, double r_t, char *curve, double S, double T,NumFunc_1 *p,double *price)
{
    double K;
...
int CALC(CF_ZCPutBondEuroHW1D)(void *Opt,void *Mod,PricingMethod *Met)
{
...
static int CHK_OPT(CF_ZCPutBondEuroHW1D)(void *Opt, void *Mod)
{
