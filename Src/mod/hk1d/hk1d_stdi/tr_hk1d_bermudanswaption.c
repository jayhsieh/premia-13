...
static int bermudanswaption_hk1d(int flat_flag,double a,double t0, double sigma_HW,double r_flat,char *curve,double T_final,double T0,NumFunc_1 *p,int am,double Nominal,double K,double per/*,int n*/,long N_step,int xnumber,double *price/*,double *delta*/)
{  
  // flat_flag           : flag to decide wether initial yield curve is flat at r_flat (0) or read from the file init (1)
  // a, sigma_HW         : parameters of the HW-model representing the market ("a" is common with the HK-process)
...
int CALC(TR_BERMUDANSWAPTION)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return bermudanswaption_hk1d(ptMod->flat_flag.Val.V_INT,
