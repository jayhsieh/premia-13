...
extern "C"{
  ...
  // Pricing a european put option on a stock driven by CGMY process
  ...
  static int MonteCarlo_TankovPoirotCGMY(double S0,NumFunc_1  *p,double T,double r,double divid,double C,double G,double M,double Y,long Ntraj,int generator,double confidence,double *ptprice,double *ptdelta,double *inf_price, double *sup_price, double *inf_delta, double *sup_delta)
  {       
    double K;
    int type;
    double price,delta,stdprice,stddelta;
    double alphap,alphan,lambdap,lambdan,cp,cn;
  ...
  int CALC(MC_TankovPoirot_CGMY)(void*Opt,void *Mod,PricingMethod *Met)
  {
    ...
    return  MonteCarlo_TankovPoirotCGMY(ptMod->S0.Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_1,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid, ptMod->C.Val.V_PDOUBLE,ptMod->G.Val.V_PDOUBLE,ptMod->M.Val.V_PDOUBLE,ptMod->Y.Val.V_PDOUBLE,Met->Par[0].Val.V_LONG,Met->Par[1].Val.V_ENUM.value,Met->Par[2].Val.V_PDOUBLE,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE),&(Met->Res[2].Val.V_DOUBLE),&(Met->Res[3].Val.V_DOUBLE),&(Met->Res[4].Val.V_DOUBLE),&(Met->Res[5].Val.V_DOUBLE));
  }

static int CHK_OPT(MC_TankovPoirot_CGMY)(void *Opt, void *Mod)
  {
  ...
  PricingMethod MET(MC_TankovPoirot_CGMY)=
  {
    "MC_TankovPoirot_CGMY",
    {{"N iterations",LONG,{100},ALLOW},
     {"RandomGenerator",ENUM,{100},ALLOW},
      {"Confidence Value",DOUBLE,{100},ALLOW},
     {" ",PREMIA_NULLTYPE,{0},FORBID}},
    CALC(MC_TankovPoirot_CGMY),
