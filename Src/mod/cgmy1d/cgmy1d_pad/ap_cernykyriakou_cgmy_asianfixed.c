...
static dcomplex CGMY_laplace_transform(dcomplex u,double t,double C,double G,double M,double Y,double drift)
{
  dcomplex temp;
  ...
  {
    temp=RCmul(C*pnl_sf_gamma(-Y),Cadd(RCmul(POW(M,Y),Csub(Cpow_real(RCsub(1,CRdiv(u,M)),Y),RCsub(1,CRdiv(CRmul(u,Y),M)))),RCmul(POW(G,Y),
    Csub(Cpow_real(RCadd(1,CRdiv(u,G)),Y),RCadd(1,CRdiv(CRmul(u,Y),G))))));
  }  
...
int cgmy_ap_cernykyriakou_asianfixed(NumFunc_2*P,double S0,double T,double r,double divid,double C,double G,double M,double Y,int n_points,double *ptprice)
{   
   double *lambda,drift,*L,*L_bar,*u,*U,*U_bar,rho,u_max,*x,*y,x_step,y_step,time_step;
   double u_step,*u_minus,*h,*a,*b,exp_X,u_em,l_em,K,coef;
   ...
   drift=(r-divid)-log(Creal(CGMY_laplace_transform(Complex(1,0),1,C,G,M,Y,0)));
   u_max=5.;
   while(Cabs(CGMY_laplace_transform(Complex(0,u_max),time_step,C,G,M,Y,drift))+Cabs(CGMY_laplace_transform(Complex(0,-u_max),time_step,C,G,M,Y,drift))>rho)
   {
     u_max+=5.;
   }
   u_step=2*u_max/u_nb;
   ...
   for(i=0;i<=u_nb;i++)
   {
     u[i]=-u_max+i*u_step;
     u_minus[i]=-u[i];
     phi[i]=CGMY_laplace_transform(Complex(0,u[i]),time_step,C,G,M,Y,drift);
   }
   /*Call Case*/       
...
///////////////////////////////////////////////////////////////////////////////
     ...
     exp_X=Creal(CGMY_laplace_transform(Complex(1,0),time_step,C,G,M,Y,drift));
...
int CALC(AP_CernyKyriakou_CGMY_FixedAsian)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  cgmy_ap_cernykyriakou_asianfixed(ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->C.Val.V_PDOUBLE,ptMod->G.Val.V_DOUBLE,ptMod->M.Val.V_SPDOUBLE,ptMod->Y.Val.V_PDOUBLE,Met->Par[0].Val.V_PINT,&(Met->Res[0].Val.V_DOUBLE));
}

static int CHK_OPT(AP_CernyKyriakou_CGMY_FixedAsian)(void *Opt, void *Mod)
{
...
static int MET(Init)(PricingMethod *Met,Option *Mod)
{
  if ( Met->init == 0)
    {
      Met->init=1;
       Met->HelpFilenameHint = "ap_cernykyriakou_cgmy_asianfixed";
      Met->Par[0].Val.V_PINT=10;
    }
  return OK;
}

PricingMethod MET(AP_CernyKyriakou_CGMY_FixedAsian)=
{
  "AP_CernyKyriakou_CGMY_FixedAsian",
  {{"Number of discretization steps",LONG,{100},ALLOW},{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(AP_CernyKyriakou_CGMY_FixedAsian),
  {{"Price",DOUBLE,{100},FORBID},{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(AP_CernyKyriakou_CGMY_FixedAsian),
  CHK_ok,
  MET(Init)
} ;
