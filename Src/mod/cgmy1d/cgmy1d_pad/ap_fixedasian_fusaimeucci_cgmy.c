...
static int FusaiMeucciCGMY_FixedAsian(double pseudo_stock,double pseudo_strike,NumFunc_2  *po,double t,double r,double divid,double C,double G,double M,double Y,int N,int M_int,double *ptprice,double *ptdelta)
{
  ...
  price=dvector(0,M_int-1);
  solution=dvector(0,M_int-1);
  
  /*  Call Price */
  CTtK=Asian_CGMY_FusaiMeucci(pseudo_stock,pseudo_strike,t,r,divid,C,G,M,Y,N,lowlim,uplim,M_int, nfft,price,solution,&delta);
  ...
  free_dvector(price,0,M_int-1);
  free_dvector(solution,0,M_int-1);
  
  return OK;
}

int CALC(AP_FixedAsian_FusaiMeucciCGMY)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  /* Case t_0 <= T_0 */
	...
	return_value= FusaiMeucciCGMY_FixedAsian(pseudo_spot,pseudo_strike,ptOpt->PayOff.Val.V_NUMFUNC_2,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->C.Val.V_PDOUBLE,ptMod->G.Val.V_PDOUBLE,ptMod->M.Val.V_PDOUBLE,ptMod->Y.Val.V_PDOUBLE,Met->Par[0].Val.V_INT2,Met->Par[1].Val.V_INT2,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE));
    }
  
  return return_value;
}

static int CHK_OPT(AP_FixedAsian_FusaiMeucciCGMY)(void *Opt, void *Mod)
{
...
PricingMethod MET(AP_FixedAsian_FusaiMeucciCGMY)=
{
  "AP_FixedAsian_FusaiMeucci_CGMY",
  {{"Nb.of Monitoring Dates",INT2,{2000},ALLOW },
   {"Nb.of Integration Points ",INT2,{1000},ALLOW},
   {" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(AP_FixedAsian_FusaiMeucciCGMY),
  {{"Price",DOUBLE,{100},FORBID},{"Delta",DOUBLE,{100},FORBID} ,{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(AP_FixedAsian_FusaiMeucciCGMY),
  CHK_ok,
  MET(Init)
};
