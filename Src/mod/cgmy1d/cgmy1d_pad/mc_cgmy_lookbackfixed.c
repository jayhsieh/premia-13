...
static int CGMY_Mc_FixedLookback(double s_maxmin,NumFunc_2*P,double S0,double T,double r,double divid,double C,double G,double M,double Y,int generator,int n_points,long n_paths,double *ptprice,double *ptdelta,double *priceerror,double *deltaerror)
{
    double eps,s,s1,s2,s3,s4,s5,s6,sup,inf,infS,supS,payoff,control,proba,discount, drift,err,K;
    double sigma,lambda_p,control_expec,lambda_m,cdf_jump_bound,pas,min_M_G,*t,cov_payoff_control;
    double var_payoff,var_control,cor_payoff_control,control_coef,var_proba,*cdf_jump_points;
    double *cdf_jump_vect_p,*cdf_jump_vect_m,*X,tau,*jump_time_vect_p,*jump_time_vect_m,*W,s0,beta;
    ...
    if(M<1 || G<1 || Y>=2 || Y==0)
    { 
     printf("Function CGMY_MC_LookbackFixed : invalid parameters. We must have M>=1, G>=1, 0<Y<2\n");
    }
...
////////////////////////////////////////////////////
    ...
    if(Y==1)
      ...
      drift=(r-divid)-C*pnl_tgamma(-Y)*(pow(M,Y)*(pow(1-1/M,Y)-1+Y/M)+pow(G,Y)*(pow(1+1/G,Y)-1-Y/G));
...
////////////////////////////////////////////////////
  ...
  if ((P->Compute)==&Call_OverSpot2)
  {
        ...
        s_maxmin=exp(-beta*sigma*sqrt(T/n_points))*s_maxmin;//shifting the predetermine maximum in order to approximate the continuous price
        for(i=0;i<n_paths;i++)
        { 
...
///////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 {
	  W[k]=sigma*pnl_rand_normal(generator)*sqrt(t[k]-t[k-1])+drift*(t[k]-t[k-1])+W[k-1];
	  N_p[k]=N_p[k-1];
	  j=N_p[k-1]+1;
	  ...
	  for(j=N_p[k-1]+1;j<=N_p[k];j++)
	    s0+=jump_generator_CGMY(cdf_jump_vect_p,cdf_jump_points,cdf_jump_vect_size,M,Y,generator);
	  N_m[k]=N_m[k-1];
	  j=N_m[k-1]+1;
	  ...
	  s0-=jump_generator_CGMY(cdf_jump_vect_m,cdf_jump_points,cdf_jump_vect_size,G,Y,generator);
...
///////////////////////////////////////////////////////////////////////////////////////////
        ...
        *ptprice=exp(beta*sigma*sqrt(T/n_points))*(s1/n_paths-control_coef*(s/n_paths-control_expec))-K*exp(-r*T);
   ...
   if ((P->Compute)==&Put_OverSpot2)
   {
        ...
        s_maxmin=exp(beta*sigma*sqrt(T/n_points))*s_maxmin;//shifting the predetermine minimum in order to approximate the continuous price
        for(i=0;i<n_paths;i++)
        { 
         //simulation of the positive jump times and number
         tau=-1/(lambda_p)*log(pnl_rand_uni(generator));
...
///////////////////////////////////////////////////////////////////////////////////////////
        ...
        *ptprice=-exp(-beta*sigma*sqrt(T/n_points))*(s1/(double)n_paths-control_coef*(s/n_paths-control_expec))+K*exp(-r*T);
	*priceerror=exp(-beta*sigma*sqrt(T/n_points))*1.96*sqrt(var_payoff*(1-cor_payoff_control*cor_payoff_control))/sqrt((double)n_paths);
...
int CALC(MC_CGMY_FixedLookback)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  CGMY_Mc_FixedLookback((ptOpt->PathDep.Val.V_NUMFUNC_2)->Par[4].Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->C.Val.V_PDOUBLE,ptMod->G.Val.V_DOUBLE,ptMod->M.Val.V_SPDOUBLE,ptMod->Y.Val.V_PDOUBLE,Met->Par[0].Val.V_ENUM.value,Met->Par[1].Val.V_PINT,Met->Par[2].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE),&(Met->Res[2].Val.V_DOUBLE),&(Met->Res[3].Val.V_DOUBLE));
}

static int CHK_OPT(MC_CGMY_FixedLookback)(void *Opt, void *Mod)
{
