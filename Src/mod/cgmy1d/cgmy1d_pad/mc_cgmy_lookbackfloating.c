...
static int CGMY_Mc_FloatingLookback(double s_maxmin,NumFunc_2*P,double S0,double T,double r,double divid,double C,double G,double M,double Y,int generator,int n_points,long n_paths,double *ptprice,double *ptdelta,double *priceerror,double *deltaerror)
{
    ...
    if(M<2 || G<=0 || Y>=2 || Y==0)
    { 
     printf("Function CGMY_MC_LookbackFloating : invalid parameters. We must have M>=2, G>0, 0<Y<2\n");
    }
...
///////////////////////////////////////////////////////////////////////////////////////////
        ...
        *ptprice=-exp(-beta*sigma*sqrt(T/n_points))*(s1/(double)n_paths-control_coef*(s/n_paths-control_expec))+S0*exp(-divid*T);
	...
	*ptprice=exp(beta*sigma*sqrt(T/n_points))*(s1/n_paths-control_coef*(s/n_paths-control_expec))-S0*exp(-divid*T);
...
int CALC(MC_CGMY_FloatingLookback)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  CGMY_Mc_FloatingLookback((ptOpt->PathDep.Val.V_NUMFUNC_2)->Par[4].Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->C.Val.V_PDOUBLE,ptMod->G.Val.V_DOUBLE,ptMod->M.Val.V_SPDOUBLE,ptMod->Y.Val.V_PDOUBLE,Met->Par[0].Val.V_ENUM.value,Met->Par[1].Val.V_PINT,Met->Par[2].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE),&(Met->Res[2].Val.V_DOUBLE),&(Met->Res[3].Val.V_DOUBLE));
}
