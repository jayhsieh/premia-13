...
int cgmy_ap_cernykyriakou_asianfloating(NumFunc_2*P,double S0,double T,double r,double divid,double C,double G,double M,double Y,int n_points,double *ptprice)
{   
   double *lambda,drift,*L,*L_bar,*u,*U,*U_bar,rho,u_max,*x,*y,x_step,y_step,time_step;
   double u_step,*u_minus,*h,*a,*b,exp_X,u_em,l_em,coef,temp_db;
...
//////////////////////////////////////////////////////////////////////////       
   ...
   {
     if(r!=divid)
       *ptprice=S0*exp(-divid*T)*Creal(q[0])+S0*exp(-divid*T)-S0*(exp(-divid*T)-exp(-r*T))/((r-divid)*T);
     else
       *ptprice=S0*exp(-divid*T)*Creal(q[0])+S0*(exp(-divid*T)-exp(-r*T));
   }     
   /*Put Case*/
...
int CALC(AP_CernyKyriakou_CGMY_FloatingAsian)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  cgmy_ap_cernykyriakou_asianfloating(ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->C.Val.V_PDOUBLE,ptMod->G.Val.V_DOUBLE,ptMod->M.Val.V_SPDOUBLE,ptMod->Y.Val.V_PDOUBLE,Met->Par[0].Val.V_PINT,&(Met->Res[0].Val.V_DOUBLE));
}
