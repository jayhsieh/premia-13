...
static int CGMY_Mc_FixedAsian(NumFunc_2*P,double S0,double T,double r,double divid,double C,double G,double M,double Y,int generator,int n_paths,double *ptprice,double *ptdelta,double *priceerror,double *deltaerror)
{   
    ...
    if(M<1 || G<1 || Y>=2 || Y==0)
    { 
     printf("Function CGMY_Mc_FixedAsian: invalid parameters. We must have M>=1, G>=1, 0<Y<2\n");
    }
...
////////////////////////////////////////////////////
    sigma=sqrt(C*(pow(M,Y-2)*(pnl_sf_gamma(2-Y)-pnl_sf_gamma_inc(2-Y,eps*M))+pow(G,Y-2)*(pnl_sf_gamma(2-Y)-pnl_sf_gamma_inc(2-Y,eps*G))));
    if(Y==1)
...
int CALC(MC_CGMY_FixedAsian)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  CGMY_Mc_FixedAsian(ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->C.Val.V_PDOUBLE,ptMod->G.Val.V_DOUBLE,ptMod->M.Val.V_SPDOUBLE,ptMod->Y.Val.V_PDOUBLE,Met->Par[0].Val.V_ENUM.value,Met->Par[1].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE),&(Met->Res[2].Val.V_DOUBLE),&(Met->Res[3].Val.V_DOUBLE));
}
