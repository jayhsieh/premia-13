...
int fft_cgmy_lookbackfloating(double s_maxmin,NumFunc_2*P,double S0,double T,double r,double divid, double C_MODEL,double G_MODEL,double M_MODEL,double Y_MODEL,int n_points,long M,double *ptprice)
{ 
  double pas,d,c,drift,nu,h_temp,x_maxmin,signe;
  dcomplex h,res;
  PnlVectComplex *G;
  pas=T/n_points;
...
int CALC(FFT_CGMY_FloatingLookback)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  fft_cgmy_lookbackfloating((ptOpt->PathDep.Val.V_NUMFUNC_2)->Par[4].Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->C.Val.V_PDOUBLE,ptMod->G.Val.V_DOUBLE,ptMod->M.Val.V_SPDOUBLE,ptMod->Y.Val.V_PDOUBLE,Met->Par[0].Val.V_PINT,Met->Par[1].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE));
}

static int CHK_OPT(FFT_CGMY_FloatingLookback)(void *Opt, void *Mod)
{  
...
static int MET(Init)(PricingMethod *Met,Option *Mod)
{
  if ( Met->init == 0)
    {
      Met->init=1;
       Met->HelpFilenameHint = "ap_cgmy_lookbackfloating_fft";
...
PricingMethod MET(FFT_CGMY_FloatingLookback)=
{
  "FFT_CGMY_LookbackFloating",
  {{"Number of discretization steps",LONG,{100},ALLOW},{"N Truncation level (a power of 2)",LONG,{100},ALLOW},{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(FFT_CGMY_FloatingLookback),
  {{"Price",DOUBLE,{100},FORBID},{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(FFT_CGMY_FloatingLookback),
  CHK_ok,
  MET(Init)
} ;
