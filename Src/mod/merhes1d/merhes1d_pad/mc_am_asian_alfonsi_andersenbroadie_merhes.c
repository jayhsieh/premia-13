...
static int MC_Am_Alfonsi_AnBr(double S0, double Maturity, double r, double divid, double V0, double k, double theta, double sigma, double rho, double mu_jump, double gamma2, double lambda, long NbrMCsimulationPrimal, long NbrMCsimulationDual, long NbrMCsimulationDualInternal, int NbrExerciseDates, int NbrStepPerPeriod, int generator,  int basis_name, int DimApprox, int flag_cir, NumFunc_2 *p, double *AmOptionUpperPrice)
{
    ...
    MC_Am_Alfonsi_LoSc(p, S0, Maturity, r, divid, V0, k, theta, sigma, rho, mu_jump, gamma2, lambda, NbrMCsimulationPrimal, NbrExerciseDates, NbrStepPerPeriod, generator, basis_name, DimApprox, flag_cir, RegressionCoeffMat, &ContinuationValue_0);
    ...
    for (m=0; m<NbrMCsimulationDual; m++)
    {
        ...
        for (i=1; i<=NbrExerciseDates-2; i++)
        {
            ...
            if (ExerciceOrContinuation)
            {
                ...
                BatesSimulation_Alfonsi(flag_SpotPaths, SpotPaths_inner, flag_VarPaths, VarPaths_inner, flag_AveragePaths, AveragePaths_inner, S_t, step, r, divid, V_t, k, theta, sigma, rho, mu_jump, gamma2, lambda, NbrMCsimulationDualInternal, 2, NbrStepPerPeriod, generator, flag_cir);
...
int CALC(MC_Am_Asian_Alfonsi_AndersenBroadie_Bates)(void *Opt, void *Mod, PricingMethod *Met)
{
...
static int CHK_OPT(MC_Am_Asian_Alfonsi_AndersenBroadie_Bates)(void *Opt, void *Mod)
{
...
static int MET(Init)(PricingMethod *Met,Option *Opt)
{
    ...
    {
        Met->init=1;
        Met->HelpFilenameHint = "mc_am_asian_alfonsi_andersenbroadie_merhes";
...
PricingMethod MET(MC_Am_Asian_Alfonsi_AndersenBroadie_Bates)=
{
    "MC_Am_Asian_Alfonsi_AndersenBroadie_Bates",
    {
    ...
    CALC(MC_Am_Asian_Alfonsi_AndersenBroadie_Bates),
    {{"Price",DOUBLE,{100},FORBID}, {" ",PREMIA_NULLTYPE,{0},FORBID}},
    CHK_OPT(MC_Am_Asian_Alfonsi_AndersenBroadie_Bates),
    CHK_ok,
    MET(Init)
};
