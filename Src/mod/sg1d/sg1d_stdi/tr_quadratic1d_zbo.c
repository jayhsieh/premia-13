...
static void ZCBond_InitialPayoffSG1D(TreeShortRate* Meth, PnlVect* ZCbondPriceVect)
{
...
static void ZCOption_InitialPayoffSG1D(PnlVect* ZCbondPriceVect, PnlVect* OptionPriceVect, NumFunc_1 *p)
{
  int j;
...
static void ZCOption_BackwardIteration(TreeShortRate* Meth, ModelParameters* ModelParam, PnlVect* ZCbondPriceVect1, PnlVect* ZCbondPriceVect2,  PnlVect* OptionPriceVect1, PnlVect* OptionPriceVect2, int index_last, int index_first, NumFunc_1 *p, int Eur_Or_Am)
{
  double a ,sigma;
...
double tr_sg1d_zcoption(TreeShortRate* Meth, ModelParameters* ModelParam, ZCMarketData* ZCMarket, double T, double S, NumFunc_1 *p, double r, int Eur_Or_Am)
{
  double OptionPrice;
  int i_T;
  ...
  ZCBond_InitialPayoffSG1D(Meth, ZCbondPriceVect2);

  ZCOption_BackwardIteration(Meth, ModelParam, ZCbondPriceVect1, ZCbondPriceVect2, ZCbondPriceVect1, ZCbondPriceVect2, Meth->Ngrid, i_T, p, 0);

  ZCOption_InitialPayoffSG1D(ZCbondPriceVect2, OptionPriceVect2, p);

  ///****************** Backward computation of the option price until initial time s *******************///
  ZCOption_BackwardIteration(Meth, ModelParam, ZCbondPriceVect1, ZCbondPriceVect2, OptionPriceVect1, OptionPriceVect2, i_T, 0, p, Eur_Or_Am);
...
static int tr_zbo1d(int flat_flag,double r0,char *curve,double a,double sigma, double S,double T, NumFunc_1 *p,int am,int N_steps,double *price)
{
  ...
  *price = tr_sg1d_zcoption(&Tr, &ModelParams, &ZCMarket, T, S, p, r0, am);
...
///************************************************ PREMIA FUNCTIONS ************************************************///
int CALC(TR_ZBOSG1D)(void *Opt,void *Mod,PricingMethod *Met)
{
...
static int CHK_OPT(TR_ZBOSG1D)(void *Opt, void *Mod)
{
...
static int MET(Init)(PricingMethod *Met,Option *Opt)
{
  if ( Met->init == 0)
    {
      Met->init=1;
      Met->HelpFilenameHint = "tr_quadratic1d_zbo";
      Met->Par[0].Val.V_LONG=500;
    }
  return OK;
}

PricingMethod MET(TR_ZBOSG1D)=
{
  "TR_SquareGaussian1d1d_ZBO",
