...
static int mc_wishart2d(PnlVect *S0,NumFunc_1 *p,double t,double r,PnlVect *divid,double alpha,PnlVect *bP,PnlVect *Sigma0V,PnlVect *QP,long N,int M, int flag_scheme,int generator,double confidence,double *ptprice,double *pterror_price, double *inf_price,double *sup_price)
{
  int dim=2;
		  ...
		  for(j=1;j<= M;j++)
			{
			  ...
			  HZZ(Stmp,Ftmp,Ytmp,tmp1,dim,DT*0.5, generator);
              correlation+=DT*pnl_mat_get(Ftmp,0,1)/sqrt(pnl_mat_get(Ftmp,0,0)*pnl_mat_get(Ftmp,1,1));
              
            }
          //------------Correlation Swap Payoff calculus
          price_sample=(p->Compute)(p->Par,correlation);
...
int CALC(MC_WISHARTVOL)(void *Opt,void *Mod,PricingMethod *Met)
{
...
static int CHK_OPT(MC_WISHARTVOL)(void *Opt, void *Mod)
{
  if ((strcmp( ((Option*)Opt)->Name,"CorrelationSwap")==0))
    return OK;
  return  WRONG;
}
...
PricingMethod MET(MC_WISHARTVOL)=
{
  "MC_WishartVol2D",
