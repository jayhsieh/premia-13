...
static void initPayoffSWAPTION(struct Tree *Meth, double T1, double per, double T2,  double K, NumFunc_1 *p)
{
  int i,j,k, n, N;
  double  Swap0=0;
  double dZC0;
  ...
  /* Give the incice time for T1 the maturity of the option on the CB(T1,T1+per, ..., T2) */
  /* Rk : T2 (S0 here), the final time of the tree is set to be final maturiy  of the CBearing */
  n=indiceTime(Meth, T1);
  k=1;
  N=Meth->TSize[n];

  /* computation of B(0,T1) */
  initPayoff1_tr(Meth,T1);
  Computepayoff(Meth,T1);


  dZC0=Meth->pLQij[0][1];


  /* cumul[] will cumulate all the ZC BondPrice values, at index time n, computed in the tree to calculate de CBearing of the option */
  ...
  /* Initialization of the variable tree payoffunc[][] to zero */
  for(i=n;i>=0; i--)
    {
      for(j=0;j<Meth->TSize[i]; j++)
        {
  ...
  /* So dZC0 will be dZC0=B(0,T1)-B(0,T2) */
  dZC0=dZC0-Meth->pLQij[0][1];

  /* cumul[] initialized with the last ZC BondPrice B(T1,T2) of the CBearing */
  for(j=0;j<N; j++){cumul[j]=(1+K*per)*Meth->pLQij[n][j];}

  /* Computation of all B(T1, T1+k*per) of the CBearing and cumul in cumul[] */
  while(T1+k*per<T2)
    {
      initPayoff1_tr(Meth,T1+k*per);
      Computepayoff(Meth,T1+k*per);

      for(j=0;j<N; j++)
        {
          cumul[j]+=K*per*Meth->pLQij[n][j]; /*pLQij[n][j] is, in the tree, B(T1,T1+k*per) just computed by Computepayoff()*/
        }
      Swap0+=per*Meth->pLQij[0][1];
      k=k+1;
    }
  /* Initialization of the Payoffunc at the index time n of T1, thanks to cumul[] which is the CBearing at time T1 */
  for(j=0;j<N; j++)
    {
      Meth->Payoffunc[n][j]=cumul[j];
    }
  free(cumul);

  p->Par[0].Val.V_DOUBLE=1.;

  /*to define receiver or payer*/
  for(j=0;j<N; j++){Meth->Payoffunc[n][j]=(p->Compute)(p->Par,Meth->Payoffunc[n][j]);}

  /*Swap0 is the corresponding swap rate a time 0 */
  Swap0=dZC0/Swap0;
  /*printf("SwapATM:%lf\n",Swap0);*/
}
...
static int swaption_cirpp1d(int flat_flag,double a0,double b0,double t0, double sigma0,double rc,char *curve, double S0,double T0,NumFunc_1 *p,int am,double Nominal,double K,double periodicity,long NtY,double *price/*,double *delta*/)
{
  long Ns;
  double r0;
  ...
  rx0=rc;
  sigma=sigma0;
  Ns=NtY*(long)(S0/periodicity);
  Tr.Ngrid=Ns;

  r0=rc;


  //N=(int)floor((S0-T0)/periodicity);

    /* Flag to decide to read or not ZC BondPrice datas in "initialyields.dat" */
  ...
  /* S0 defines the final time tree variable (no time can be larger), Ns is the number of time step */
  SetTimegrid(&Tr, Ns, S0);
  /* Allocate and initialize the tree*/
  SetTree(&Tr);
  /* translate the tree by "alpha" */
  TranslateTree(&Tr, &ZCMarket);
  /* Initialize the payoff for swaption as an option on CBearing(t=T0, T0, T0+periodicity, ..., S0)*/
  initPayoffSWAPTION(&Tr, T0, periodicity, S0, K, p); /*Comments of this functions just above*/
  /* Compute the option from T0 to 0 in pLQij[][] tree variable */
  Computepayoff(&Tr,T0);

  /* return the result plQij[0][] or plQij[indiceTime(t0)][] in case of present or futur option */
  if(t0==0){*price =Nominal*OPTION(&Tr);}
  else {    *price =Nominal*OPTIONr_tr(&Tr,r0,t0);}
...
int CALC(TR_SwaptionCIRpp1D)(void *Opt,void *Mod,PricingMethod *Met)
{
  TYPEOPT* ptOpt=(TYPEOPT*)Opt;
  TYPEMOD* ptMod=(TYPEMOD*)Mod;

  return swaption_cirpp1d(ptMod->flat_flag.Val.V_INT,ptMod->a.Val.V_DOUBLE,ptMod->b.Val.V_DOUBLE,ptMod->T.Val.V_DATE,
                          ...
                          ptOpt->Nominal.Val.V_PDOUBLE,ptOpt->FixedRate.Val.V_PDOUBLE,ptOpt->ResetPeriod.Val.V_DATE,
                          Met->Par[0].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE)/*,&(Met->Res[1].Val.V_DOUBLE)*/);
}


static int CHK_OPT(TR_SwaptionCIRpp1D)(void *Opt, void *Mod)
{
...
PricingMethod MET(TR_SwaptionCIRpp1D)=
{
  "TR_CIRpp1d_Swaption",
  {{"TimeStepNumber for Period",LONG,{100},ALLOW},
   {" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(TR_SwaptionCIRpp1D),
  {{"Price",DOUBLE,{100},FORBID}/*,{"Delta",DOUBLE,{100},FORBID} */,{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(TR_SwaptionCIRpp1D),
  CHK_ok,
  MET(Init)
} ;
