...
/*Payer Swaption*/
static int ps_cirpp1d(int flat_flag,double a,double b, double date,double sigma,double rcc,char *curve, double Nominal,double K,double periodicity,double option_maturity,double contract_maturity,double *price/*,double *delta*/)
{
  ...
  for(i=0;i<=nb_payement;i++)
    {
      ...
      sum=sum+C[i]*zbput(a, b,sigma, rcc, date,option_maturity,ti, new_K, &ZCMarket);
    }
  /*Price*/
  *price=Nominal*sum;
  /*Delta*/
...
int CALC(CF_PayerSwaption)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return ps_cirpp1d(ptMod->flat_flag.Val.V_INT,ptMod->a.Val.V_DOUBLE,ptMod->b.Val.V_DOUBLE,ptMod->T.Val.V_DATE,
