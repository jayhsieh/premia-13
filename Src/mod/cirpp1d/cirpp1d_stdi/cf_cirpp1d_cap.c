...
/*Cap*/
static int cap_cirpp1d(int flat_flag,double a,double b, double date,double sigma,double rcc,char *curve, double Nominal,double K,double periodicity,double first_payement,double contract_maturity,double *price/*,double *delta*/)
{
    double sum,tim,tip;
    int i, nb_payement;
    ...
    for(i=0;i<nb_payement;i++)
    {
        ...
        tip=tim+periodicity;
        sum+=(1.+K*periodicity)*zbput(a,b,sigma,rcc,date,tim,tip,1./(1.+K*periodicity), &ZCMarket);
    }
...
int CALC(CF_Cap)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return cap_cirpp1d(ptMod->flat_flag.Val.V_INT,
