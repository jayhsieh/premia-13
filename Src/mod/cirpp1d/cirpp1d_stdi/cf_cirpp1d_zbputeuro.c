...
/*Zero Coupon Bond*/
static double zcb_cirpp1d(double rcc,int flat_flag,double a,double b,double sigma,double t,double T, ZCMarketData* ZCMarket)
{
    ...
    if(t-0.5*INC>0)
    {
        f0_t = (log( BondPrice(t-0.5*INC, ZCMarket))-log( BondPrice(t+0.5*INC, ZCMarket)))/INC;
    }
    else
    {
...
/*Call Option*/
static int zbp_cirpp1d(int flat_flag, double a, double b,double t,double sigma, double rcc, char *curve, double S, double T,NumFunc_1 *p,double *price,double *delta)
{
    double K;
...
int CALC(CF_ZCPutBondEuro)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return zbp_cirpp1d(ptMod->flat_flag.Val.V_INT,ptMod->a.Val.V_DOUBLE,ptMod->b.Val.V_DOUBLE,ptMod->T.Val.V_DATE,
