...
/*Call Option*/
static int zbc_cir1d(double r, double k,double t, double sigma,double theta, double S, double T,NumFunc_1 *p,double *price,double *delta)
{
  double K;
...
int CALC(CF_ZCCallBondEuro)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return zbc_cir1d(ptMod->r0.Val.V_PDOUBLE,ptMod->k.Val.V_DOUBLE,ptMod->T.Val.V_DATE,ptMod->Sigma.Val.V_PDOUBLE,
