...
static int capfloor_cir1d(double r0,double k,double t0, double sigma,double theta,double first_payement,double Nominal,double K,double periodicity,NumFunc_1 *p,double T,int NtY,int Ns,double cn_theta,double *price)
{
...
int CALC(FD_GaussCAPFLOOR)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return capfloor_cir1d(ptMod->r0.Val.V_PDOUBLE,ptMod->k.Val.V_DOUBLE,ptMod->T.Val.V_DATE,ptMod->Sigma.Val.V_PDOUBLE,ptMod->theta.Val.V_PDOUBLE,ptOpt->FirstResetDate.Val.V_DATE,ptOpt->Nominal.Val.V_PDOUBLE,ptOpt->FixedRate.Val.V_PDOUBLE,ptOpt->ResetPeriod.Val.V_DATE,ptOpt->PayOff.Val.V_NUMFUNC_1,ptOpt->BMaturity.Val.V_DATE,Met->Par[0].Val.V_INT,Met->Par[1].Val.V_INT,Met->Par[2].Val.V_RGDOUBLE,&(Met->Res[0].Val.V_DOUBLE));
}
...
static int MET(Init)(PricingMethod *Met,Option *Opt)
{
  if ( Met->init == 0)
    {
      Met->init=1;
      Met->HelpFilenameHint = "fd_gauss_cir1d_capfloor";
