...
static int VG_Mc_FloatingAsian(NumFunc_2*P,double S0,double
...
{     
    double s,s1,eps,err,*Xg,*Xd,*jump_time_vect_p,*jump_time_vect_m,dpayoff,lambda_p,lambda_m;
    ...
    double *cdf_jump_points,*cdf_jump_vect_p,*cdf_jump_vect_m,min_M_G,var_dpayoff,payoff;
...
////////////////////////////////////////////////////////////
         ...
         dpayoff=exp(-r*T)*(exp(Xd[jump_number+1])-payoff/(drift*T))*(payoff/(drift*T)<exp(Xd[jump_number+1]));
         payoff=exp(-r*T)*S0*(exp(Xd[jump_number+1])-payoff/(drift*T))*(payoff/(drift*T)<exp(Xd[jump_number+1]));         
         s1+=payoff;
         s+=payoff*payoff;
...
////////////////////////////////////////////////////////////
         ...
         dpayoff=exp(-r*T)*(payoff/(drift*T)-exp(Xd[jump_number+1]))*(payoff/(drift*T)>exp(Xd[jump_number+1]));
         payoff=exp(-r*T)*S0*(payoff/(drift*T)-exp(Xd[jump_number+1]))*(payoff/(drift*T)>exp(Xd[jump_number+1]));         
         s1+=payoff;
         s+=payoff*payoff;
         control=S0*exp(Xd[jump_number+1]);
         s2+=control;
...
int CALC(MC_VarianceGamma_FloatingAsian)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  VG_Mc_FloatingAsian(ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Theta.Val.V_DOUBLE,ptMod->Kappa.Val.V_SPDOUBLE,Met->Par[0].Val.V_ENUM.value,Met->Par[1].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE),&(Met->Res[2].Val.V_DOUBLE),&(Met->Res[3].Val.V_DOUBLE));
}

static int CHK_OPT(MC_VarianceGamma_FloatingAsian)(void *Opt, void *Mod)
{
...
PricingMethod MET(MC_VarianceGamma_FloatingAsian)=
{
  "MC_VG_AsianFloating",
  {{"RandomGenerator",ENUM,{100},ALLOW},
   {"N iterations",LONG,{100},ALLOW},{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(MC_VarianceGamma_FloatingAsian),
  ...
  CHK_OPT(MC_VarianceGamma_FloatingAsian),
  CHK_ok,
  MET(Init)
} ;
