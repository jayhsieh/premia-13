...
extern "C"{
  ...
  static int CarrNIG(double S0,NumFunc_1  *p,double T,double r,double divid,double sigma,double theta,double kappa,double *ptprice,double *ptdelta)
  {       
    double K;
    double Sigma=0.2;
    ...
    NIG_measure measure(theta, sigma, kappa,0.1);
  ...
  int CALC(AP_CarrNIG)(void*Opt,void *Mod,PricingMethod *Met)
  {
    ...
    return  CarrNIG(ptMod->S0.Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_1,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Theta.Val.V_DOUBLE,ptMod->Kappa.Val.V_SPDOUBLE,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE));
  }

static int CHK_OPT(AP_CarrNIG)(void *Opt, void *Mod)
  {
