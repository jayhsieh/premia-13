...
static int NIG_Mc_FloatingAsian(NumFunc_2*P,double S0,double
T,double r,double divid,double sigma,double theta,double kappa,int generator,int n_paths,double *ptprice,double *priceerror,double *ptdelta,double *deltaerror)
{   
    ...
    double alpha,beta,delta,var_temp,cov_temp,*vect_temp,g_temp;
    ...
    { 
     printf("Function NIG_Mc_FloatingAsian: invalid parameters. We must have sqrt(kappa)*(2*fabs(theta)+sigma*sigma)<=1\n");
    }
...
////////////////////////////////////////////////////////////
         ...
         control=S0*payoff/T;                                
         dpayoff=discount*(exp(Xd[jump_number+1])-payoff/T)*(payoff/T<exp(Xd[jump_number+1]));
         payoff=discount*(S0*exp(Xd[jump_number+1])-S0*payoff/T)*(payoff/T<exp(Xd[jump_number+1]));         
         s1+=payoff;
         s+=payoff*payoff;
         s2+=control;
         s3+=control*control;
...
////////////////////////////////////////////////////////////
         ...
         *ptprice=(s1/n_paths-control_coef*(s2/n_paths-control_expec))-S0*exp(-divid*T)+S0*exp(-r*T);
        *priceerror=1.96*sqrt(var_payoff*(1-cor_payoff_control*cor_payoff_control))/sqrt(n_paths);
        if(r!=divid)
...
int CALC(MC_NIG_FloatingAsian)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  NIG_Mc_FloatingAsian(ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Theta.Val.V_DOUBLE,ptMod->Kappa.Val.V_SPDOUBLE,Met->Par[0].Val.V_ENUM.value,Met->Par[1].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE),&(Met->Res[2].Val.V_DOUBLE),&(Met->Res[3].Val.V_DOUBLE));
}

static int CHK_OPT(MC_NIG_FloatingAsian)(void *Opt, void *Mod)
{  
...
PricingMethod MET(MC_NIG_FloatingAsian)=
{
  "MC_NIG_AsianFloating",
  {{"RandomGenerator",ENUM,{100},ALLOW},  {"N iterations",LONG,{100},ALLOW},{" ",PREMIA_NULLTYPE,{0},FORBID}},  CALC(MC_NIG_FloatingAsian),
