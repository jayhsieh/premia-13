...
int nig_ap_cernykyriakou_asianfloating(NumFunc_2*P,double S0,double T,double r,double divid,double sigma,double theta,double kappa,int n_points,double *ptprice)
{   
   double *lambda,drift,*L,*L_bar,*u,*U,*U_bar,rho,u_max,*x,*y,x_step,y_step,time_step;
   double u_step,*u_minus,*h,u_em,l_em,*a,*b,exp_X,alpha,beta,delta,coef;
...
int CALC(AP_CernyKyriakou_NIG_FloatingAsian)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  nig_ap_cernykyriakou_asianfloating(ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Theta.Val.V_DOUBLE,ptMod->Kappa.Val.V_SPDOUBLE,Met->Par[0].Val.V_PINT,&(Met->Res[0].Val.V_DOUBLE));
}

static int CHK_OPT(AP_CernyKyriakou_NIG_FloatingAsian)(void *Opt, void *Mod)
{
...
static int MET(Init)(PricingMethod *Met,Option *Mod)
{
  if ( Met->init == 0)
    {
      Met->init=1;
      Met->HelpFilenameHint = "ap_cernykyriakou_nig_asianfloating";
      Met->Par[0].Val.V_PINT=10;
    }
  return OK;
}

PricingMethod MET(AP_CernyKyriakou_NIG_FloatingAsian)=
{
  "AP_CernyKyriakou_NIG_FloatingAsian",
  {{"Number of discretization steps",LONG,{100},ALLOW},{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(AP_CernyKyriakou_NIG_FloatingAsian),
  {{"Price",DOUBLE,{100},FORBID},{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(AP_CernyKyriakou_NIG_FloatingAsian),
  CHK_ok,
  MET(Init)
} ;
