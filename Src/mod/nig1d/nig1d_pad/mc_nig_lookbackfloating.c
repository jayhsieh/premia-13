...
static int NIG_Mc_Floating(double s_maxmin,NumFunc_2*P,double S0,double T,double r,double divid,double sigma,double theta,double kappa,int generator,int n_points,long n_paths,double *ptprice,double *ptdelta,double *priceerror,double *deltaerror)
{
    double eps,s,s1,s2,s3,s4,s5,s6,sup,inf,infS,supS,payoff,control,proba,discount, drift,err; 
    double sigma0,lambda_p,control_expec,lambda_m,cdf_jump_bound,pas,*t,cov_payoff_control,var_payoff,var_control;
    double cor_payoff_control,control_coef,var_proba,*cdf_jump_points,*cdf_jump_vect_p,*cdf_jump_vect_m,*X,tau;
    ...
    { 
     printf("Function NIG_Mc_Floating: invalid parameters. We must have sqrt(kappa)*(2*fabs(theta)+sigma*sigma)<=1\n");
    }
...
///////////////////////////////////////////////////////////////////////////////////////////
        ...
        *ptprice=-exp(-beta1*sigma0*sqrt(T/n_points))*(s1/(double)n_paths-control_coef*(s/n_paths-control_expec))+S0*exp(-divid*T);
...
///////////////////////////////////////////////////////////////////////////////////////////
        ...
        *ptprice=exp(beta1*sigma0*sqrt(T/n_points))*(s1/n_paths-control_coef*(s/n_paths-control_expec))-S0*exp(-divid*T);
...
int CALC(MC_Nig_Floating)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  NIG_Mc_Floating((ptOpt->PathDep.Val.V_NUMFUNC_2)->Par[4].Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Theta.Val.V_DOUBLE,ptMod->Kappa.Val.V_SPDOUBLE,Met->Par[0].Val.V_ENUM.value,Met->Par[1].Val.V_PINT,Met->Par[2].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE),&(Met->Res[2].Val.V_DOUBLE),&(Met->Res[3].Val.V_DOUBLE));
}
