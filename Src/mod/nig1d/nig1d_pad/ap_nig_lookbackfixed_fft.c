...
int fft_NIG_lookbackfixed(double s_maxmin,NumFunc_2*P,double S0,double T,double r,double divid,double sigma,double theta,double kappa,int n_points,long M,double *ptprice)
{ 
  double pas,d,c,drift,nu,h_temp,x_maxmin,signe,alpha,beta,delta,K;
  dcomplex h,res;
  PnlVectComplex *G;
...
int CALC(FFT_NIG_FixedLookback)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  fft_NIG_lookbackfixed((ptOpt->PathDep.Val.V_NUMFUNC_2)->Par[4].Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Theta.Val.V_DOUBLE,ptMod->Kappa.Val.V_SPDOUBLE,Met->Par[0].Val.V_PINT,Met->Par[1].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE));
}

static int CHK_OPT(FFT_NIG_FixedLookback)(void *Opt, void *Mod)
{  
...
static int MET(Init)(PricingMethod *Met,Option *Mod)
{
  if ( Met->init == 0)
    {
      Met->init=1;
      Met->HelpFilenameHint = "ap_nig_lookbackfixed_fft";
