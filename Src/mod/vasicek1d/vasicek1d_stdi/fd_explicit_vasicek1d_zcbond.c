...
static int bond_vasicek1d(double r0,double k,double t0, double sigma,double theta,double T,long Nt,double *price)
{
  int j;
  double val,val1;
...
int CALC(FD_ZCBOND)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return bond_vasicek1d(ptMod->r0.Val.V_PDOUBLE,ptMod->k.Val.V_DOUBLE,ptMod->T.Val.V_DATE,ptMod->Sigma.Val.V_PDOUBLE,
