...
static double zbc_vasicek1d(double t,double T,double S,double new_K,double r,double theta,double k,double sigma)
{
  double PtS,PtT;
...
/*Receiver Swaption*/
static int rc_vasicek1d(double r,double k, double date,double sigma,double theta,double Nominal,double K,double periodicity,double option_maturity,double contract_maturity,double *price)
{
  ...
  /*Receiver Swaption=Call Option on Coupon-Bearing Bond*/
  ...
  for(i=0;i<=nb_payement;i++)
    {
      ...
      sum+=C[i]*zbc_vasicek1d(date,option_maturity,ti,new_K,r,theta,k,sigma);
    }
...
int CALC(CF_ReceiverSwaption)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return rc_vasicek1d(ptMod->r0.Val.V_PDOUBLE,ptMod->k.Val.V_DOUBLE,ptMod->T.Val.V_DATE,ptMod->Sigma.Val.V_PDOUBLE,
