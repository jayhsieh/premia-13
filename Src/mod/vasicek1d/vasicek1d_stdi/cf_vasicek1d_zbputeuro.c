...
/*Put Option*/
static int zbp_vasicek1d(double r, double k,double t, double sigma,double theta, double S, double T,NumFunc_1 *p,double *price,double *delta)
{
  double PtS,PtT;
  ...
  /*Price*/
  *price=K*PtT*cdf_nor(-d2)-PtS*cdf_nor(-d1);
...
int CALC(CF_ZCPutBondEuro)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return zbp_vasicek1d(ptMod->r0.Val.V_PDOUBLE,ptMod->k.Val.V_DOUBLE,ptMod->T.Val.V_DATE,ptMod->Sigma.Val.V_PDOUBLE,ptMod->theta.Val.V_PDOUBLE,ptOpt->BMaturity.Val.V_DATE,ptOpt->OMaturity.Val.V_DATE,ptOpt->PayOff.Val.V_NUMFUNC_1,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE));
}
