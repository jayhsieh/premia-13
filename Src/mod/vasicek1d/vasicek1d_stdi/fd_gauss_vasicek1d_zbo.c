...
/*Option Computation*/
static int zbo_vasicek1d(double r0,double k,double t0, double sigma,double theta,double T,double t,NumFunc_1 *p,int am,int Nt,int Ns,double cn_theta,double *price/*,double *delta*/)
{
  int i,j,Nt0;
...
int CALC(FD_GaussZBO)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return zbo_vasicek1d(ptMod->r0.Val.V_PDOUBLE,ptMod->k.Val.V_DOUBLE,ptMod->T.Val.V_DATE,ptMod->Sigma.Val.V_PDOUBLE,ptMod->theta.Val.V_PDOUBLE,ptOpt->BMaturity.Val.V_DATE,ptOpt->OMaturity.Val.V_DATE,ptOpt->PayOff.Val.V_NUMFUNC_1,ptOpt->EuOrAm.Val.V_BOOL,Met->Par[0].Val.V_INT,Met->Par[1].Val.V_INT,Met->Par[2].Val.V_RGDOUBLE,&(Met->Res[0].Val.V_DOUBLE)/*,&(Met->Res[1].Val.V_DOUBLE)*/);
}
