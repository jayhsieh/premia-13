...
static int cb_vasicek(int Nt,int Nt0,int Ns,double K,double periodicity,double first_payement,int nb_coupon)
{
  int i,z,TimeIndex;
...
static int swaption_vasicek1d(double r0,double k,double t0, double sigma,double theta,double T,double t,NumFunc_1 *p,int am,double Nominal,double K,double periodicity,long NtY,int Ns,double cn_theta,double *price)
{
  ...
  cb_vasicek(Nt,Nt0,Ns,K,periodicity,first_payement,nb_coupon);
...
int CALC(FD_GaussSWAPTION)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return swaption_vasicek1d(ptMod->r0.Val.V_PDOUBLE,ptMod->k.Val.V_DOUBLE,ptMod->T.Val.V_DATE,ptMod->Sigma.Val.V_PDOUBLE,
