...
//defintion du premier instant où on passe au dessus de la
...
static void theta(PnlVectInt *res, PnlMat *asset, int M, int N, param *P)
{
  ...
  for(j=0;j<M;j++)
    {
      i=0;
      while((pnl_mat_get(asset,i,j)<P->Sbarre-eps)&&(i<N)) i++;
...
static double prix_standard_protection(int M, int N, double spot, double T, int gen, int bindex, int m, param *P)
{
  PnlMat *asset;
  PnlBasis *basis;
  double sol;
 ...
 * an integer
 */
int callable_std_protection (double *prix, TYPEMOD *Mod, TYPEOPT *Opt, int gen, int bindex, int m, int M, int steps)
{
  param *P;
  ...
  *prix = prix_standard_protection(M,N,spot,T,gen,bindex,m,P);
  free(P);
  return OK;;
}
