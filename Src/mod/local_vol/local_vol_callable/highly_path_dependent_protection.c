...
typedef struct
{
  ...
  double R;
  double cbarre;
  double l;//nb de jours (cumulés) où il faut être resté au
           //dessus de Sbarre durant les d dernières dates
           //de monitoring
  double d;//nb de dates de monitoring avant t  (toujours
           //>=l et <=N_trading+1)
} param;
...
//stocke dans res le nom de dates (en cumulé) passées au
//dessus de Sbarre depuis d dates
static void calcul_mod_H(PnlMatInt *res, PnlMatInt *H, int M, int N, int N_trading, param *P)
{
  int i,j;
  pnl_mat_int_resize(res,N_trading+1,M);
  pnl_mat_int_cumsum(H,'r');
  for(j=0;j<M;j++)
    {
      for(i=0;i<P->d;i++) pnl_mat_int_set(res,i,j,pnl_mat_int_get(H,i,j));
      for(i=P->d;i<N_trading+1;i++) pnl_mat_int_set(res,i,j,pnl_mat_int_get(H,i,j)-pnl_mat_int_get(H,i-P->d,j));
    }
}

//defintion du premier instant où H dépasse l
static void theta(PnlVectInt *res, PnlMatInt *mod_H, int M, int N, int N_trading, param *P)
{
  int j,i;
  int coef=N/N_trading;
  ...
  for(j=0;j<M;j++)
    {
      i=0;
      while((pnl_mat_int_get(mod_H,i,j)<P->l-eps)&&(i<N_trading)) i++;
 ...
 * an integer
 */
int callable_highly_path_dep_protection(double *prix, TYPEMOD *Mod, TYPEOPT *Opt, int gen, int bindex, int m,  int M, int steps)
{
  param *P;
  ...
  P->d=Opt->Period.Val.V_PINT;
  P->l=Opt->Window.Val.V_PINT;
  pnl_rand_init(gen, M, N);                              
                                           

  *prix = prix_highly_path_dependent_protection(M,N,N_trading,spot,T,gen,bindex,m,P);
  free(P);
  return OK;
}
