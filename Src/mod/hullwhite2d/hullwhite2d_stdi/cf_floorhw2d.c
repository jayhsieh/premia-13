...
static double cf_zbcall2d(ZCMarketData* ZCMarket,double t,double r,double u,double a,double sigma1,double b,double sigma2,double rho,double S,double T, double X)
{
    double PtS,PtT;
    double h, sigma_p;
    double price;
    ...
    price = PtS * cdf_nor(h) - X * PtT * cdf_nor(h-sigma_p);

    return price;
}


static int cf_floor2d(int flat_flag,double r_t,char *curve, double u_t,double a,double sigma1,double b,double sigma2,double rho,
                            double Nominal,double K,double periodicity,double first_payement,double contract_maturity,double *price)
{
  ...
  for(i=0; i<nb_payement; i++)
  {
      ...
      tip   = tim + periodicity;
      sum  += cf_zbcall2d(&ZCMarket, 0, r_t, u_t, a, sigma1, b, sigma2, rho, tip, tim, strike_put);
  }
...
int CALC(CF_FLOORHW2D)(void *Opt,void *Mod,PricingMethod *Met)
{
  TYPEOPT* ptOpt=(TYPEOPT*)Opt;
  TYPEMOD* ptMod=(TYPEMOD*)Mod;

  return  cf_floor2d(   ptMod->flat_flag.Val.V_INT,
                        MOD(GetYield)(ptMod),
                        MOD(GetCurve)(ptMod),
...
static int CHK_OPT(CF_FLOORHW2D)(void *Opt, void *Mod)
{
...
PricingMethod MET(CF_FLOORHW2D)=
{
  "CF_FloorHW2D",
  {{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(CF_FLOORHW2D),
  {{"Price",DOUBLE,{100},FORBID}/*,{"Delta",DOUBLE,{100},FORBID} */,{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(CF_FLOORHW2D),
  CHK_ok,
  MET(Init)
} ;
