...
static int cf_zbcall2d(int flat_flag,double r,char *curve,double u,double a,double sigma1,double b,double sigma2,double rho,double S,double T,NumFunc_1 *p,double *price)
{
    double PtS,PtT, X;
    double h, sigma_p;
...
int CALC(CF_ZBCALLHW2D)(void *Opt,void *Mod,PricingMethod *Met)
{
  TYPEOPT* ptOpt=(TYPEOPT*)Opt;
  TYPEMOD* ptMod=(TYPEMOD*)Mod;

  return  cf_zbcall2d(  ptMod->flat_flag.Val.V_INT,
                        MOD(GetYield)(ptMod),
                        MOD(GetCurve)(ptMod),
...
static int CHK_OPT(CF_ZBCALLHW2D)(void *Opt, void *Mod)
{
...
PricingMethod MET(CF_ZBCALLHW2D)=
{
  "CF_ZBCallEuroHW2D",
  {{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(CF_ZBCALLHW2D),
  {{"Price",DOUBLE,{100},FORBID}/*,{"Delta",DOUBLE,{100},FORBID} */,{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(CF_ZBCALLHW2D),
  CHK_ok,
  MET(Init)
} ;
