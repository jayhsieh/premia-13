...
static int MCMerton(double s, NumFunc_1  *p, double t, double r, double divid, double sigma,double lambda,double mu,double gamma2,long N, int generator, double inc, double confidence, double *ptprice, double *ptdelta, double *pterror_price, double *pterror_delta , double *inf_price, double *sup_price, double *inf_delta, double *sup_delta)
{
  long i;
  double mean_price, mean_delta, var_price, var_delta, forward, forward_stock, price_sample_plus, price_sample_minus,exp_sigmaxwt, S_T, U_T, price_sample, delta_sample=0., s_plus, s_minus,sigma_sqrt;
  double g;
  int init_mc;
  ...
  if(init_mc == OK)
      ...
      for(i=1 ; i<=N ; i++)
	{
	  ...
	  delta_sample= (price_sample_plus-price_sample_minus)/(2.*s*inc);		
	  /*Sum*/
...
int CALC(MC_Merton)(void *Opt, void *Mod, PricingMethod *Met)
{
