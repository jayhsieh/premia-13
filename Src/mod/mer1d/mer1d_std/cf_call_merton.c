...
static int Call_Merton(double x,NumFunc_1  *p,double T,double r,double divid,double sigma,double lambda,double m,double v,double *ptprice,double *ptdelta)
{ 
  ...
  d2=d1-sigmasqrt;
  price=x*M*cdf_nor(d1)-K*exp(-rT-lambdaT)*cdf_nor(d2);
  puissancen1=1.;
  factorieln=1.;
  delta=M*cdf_nor(d1);
  ...
  while (test<0.99999)
    {n++;
      ...
      d2=d1-sigmasqrt;
      price+=(puissancen1/factorieln)*(x*exp(n*mv2)*M*cdf_nor(d1)-K*ex_r_lT*cdf_nor(d2));
...
int CALC(CF_Call_Merton)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  Call_Merton(ptMod->S0.Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_1,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,
...
static int CHK_OPT(CF_Call_Merton)(void *Opt, void *Mod)
{
  /* 
...
PricingMethod MET(CF_Call_Merton)=
{
  "CF_Call_Merton",
  {{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(CF_Call_Merton),
  {{"Price",DOUBLE,{100},FORBID},{"Delta",DOUBLE,{100},FORBID},{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(CF_Call_Merton),
  CHK_ok,
  MET(Init)
} ;
