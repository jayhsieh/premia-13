...
extern "C"{
  static int CarrMadanMer(double S0,NumFunc_1  *p,double T,double r,double divid,double sigma,double lambda,double mu,double gamma2,double *ptprice,double *ptdelta)
  {    
    double K;
    double Sigma=0.2;
    ...
    Merton_measure measure(mu,delta,lambda,sigma,0.1);
  ...
  int CALC(AP_Carr)(void*Opt,void *Mod,PricingMethod *Met)
  {
    ...
    return  CarrMadanMer(ptMod->S0.Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_1,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Lambda.Val.V_PDOUBLE,ptMod->Mean.Val.V_PDOUBLE,ptMod->Variance.Val.V_PDOUBLE,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE));
  }

static int CHK_OPT(AP_Carr)(void *Opt, void *Mod)
  {
