...
static int ImpExpDownOut(int am,double S0,NumFunc_1  *p,double l_down,double rebate,double T,double r,double divid,double sigma,double lambda,double mu,double gamma2,double dx,int M,int flag_scheme,double *ptprice,double *ptdelta)
{
...
int CALC(FD_ImpExpDownOut)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return ImpExpDownOut(ptOpt->EuOrAm.Val.V_BOOL,ptMod->S0.Val.V_PDOUBLE,
                       ptOpt->PayOff.Val.V_NUMFUNC_1, limit,rebate,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Lambda.Val.V_PDOUBLE,ptMod->Mean.Val.V_PDOUBLE,ptMod->Variance.Val.V_PDOUBLE,Met->Par[0].Val.V_DOUBLE,Met->Par[1].Val.V_INT,Met->Par[2].Val.V_ENUM.value,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE));
}
...
static int MET(Init)(PricingMethod *Met,Option *Opt)
{
  ...
  if (first) 
    {
      Met->HelpFilenameHint = "FD_ImpExpDownOut_mer";
