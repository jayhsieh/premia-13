...
static int Merton_Mc_FixedLookback(double s_maxmin,NumFunc_2*P,double S0,double T,double r,double divid,double sigma,double lambda,double mean,double var,
int generator,int n_points,long n_paths,double *ptprice,double *ptdelta,double *priceerror,double *deltaerror)
{
  double *s,s1,s2,*s3,**s4,s5,s6;
  double pas,sup,inf,supw,infw,exp_sup,exp_inf,exp_supw,exp_infw,beta;
  double *t,*jump_size_vect,*jump_time_vect,sup0,temp,nu,discount,*cov_payoff_control,var_payoff,**var_control,K;
  double cor_payoff_control,*coef_control,var_proba,determinant_control;
  double *X,*W,payoff,*control,proba,s0,s_shift,e1,e2,e3,expect_control_0, delta;
  ...
  if ((P->Compute)==&Put_OverSpot2)
    {
	 ...
	 for(k=1;k<=n_points;k++)
	 {
	   ...
	   X[k]=X[k-1]+(W[k]-W[k-1])+s0;
         ...
         payoff=exp_inf;
         control[0]=exp_infw;
         exp_inf=S0*exp(X[n_points]-sup);//antithetic variate associated with the exponential of the Levy infimum
         exp_infw=S0*exp(W[n_points]-supw);//antithetic variate associated with the first control variate
         if(exp_inf>s_shift) 
         {
          exp_inf=s_shift; 
          proba+=1.;
        ...
        *ptprice=-exp(-beta*sigma*sqrt(T/n_points))*(s1/n_paths-coef_control[0]*(s[0]/n_paths-expect_control_0)-coef_control[1]*(s[1]/n_paths-S0*exp(-divid*T)))+K*exp(-r*T);
        *priceerror=exp(-beta*sigma*sqrt(T/n_points))*1.96*sqrt(var_payoff*(1-cor_payoff_control*cor_payoff_control))/sqrt(n_paths);
        *ptdelta=(*ptprice+(S0*exp(-divid*T)-K*exp(-r*T))+discount*s_maxmin*s5/n_paths)/S0-exp(-divid*T);
    ...
    if ((P->Compute)==&Call_OverSpot2)
      {
	   ...
	   for(k=1;k<=n_points;k++)
	   {
	     ...
	     X[k]=X[k-1]+(W[k]-W[k-1])+s0;
           ...
           payoff=exp_sup;
           control[0]=exp_supw;
           exp_sup=S0*exp(X[n_points]-inf);//antithetic variate associated with the exponential of the Levy supremum
           exp_supw=S0*exp(W[n_points]-infw);//antithetic variate associated with the first control variate
           if(exp_sup<s_shift) 
           {
            exp_sup=s_shift; 
            proba+=1.;
...
int CALC(MC_Merton_FixedLookback)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  Merton_Mc_FixedLookback((ptOpt->PathDep.Val.V_NUMFUNC_2)->Par[4].Val.V_PDOUBLE,
                                  ptOpt->PayOff.Val.V_NUMFUNC_2,
                                  ptMod->S0.Val.V_PDOUBLE,
                                  ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,
                                  r,divid,ptMod->Sigma.Val.V_PDOUBLE,
                                  ptMod->Lambda.Val.V_PDOUBLE,ptMod->Mean.Val.V_PDOUBLE,ptMod->Variance.Val.V_PDOUBLE,
                                  Met->Par[0].Val.V_ENUM.value,Met->Par[1].Val.V_PINT,Met->Par[2].Val.V_LONG,
                                  &(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE),&(Met->Res[2].Val.V_DOUBLE),&(Met->Res[3].Val.V_DOUBLE));
}

static int CHK_OPT(MC_Merton_FixedLookback)(void *Opt, void *Mod)
{
...
PricingMethod MET(MC_Merton_FixedLookback)=
{
  ...
  CHK_OPT(MC_Merton_FixedLookback),
  CHK_ok,
  MET(Init)
} ;
