...
static int Merton_Mc_FloatingLookback(double s_maxmin,NumFunc_2*P,double S0,double T,double r,double divid,double sigma,                                    double lambda,double mean,double var,int generator,int n_points,long n_paths,double *ptprice,double *ptdelta,double *priceerror,double *deltaerror)
{
    ...
    if ((P->Compute)==&Put_StrikeSpot2)
      {
	   ...
	   for(k=1;k<=n_points;k++)
	   {
	     ...
	     X[k]=X[k-1]+(W[k]-W[k-1])+s0;
           ...
           payoff=exp_sup;
           control[0]=exp_supw;
           exp_sup=S0*exp(X[n_points]-inf);//antithetic variat associated with the exponential of the Levy supremum
...
int CALC(MC_Merton_FloatingLookback)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  Merton_Mc_FloatingLookback((ptOpt->PathDep.Val.V_NUMFUNC_2)->Par[4].Val.V_PDOUBLE,
