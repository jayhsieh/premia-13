...
static int FusaiMeucciMER_FixedAsian(double pseudo_stock,double pseudo_strike,NumFunc_2  *po,double t,double r,double divid,double sigma,double lambda,double mu,double gamma2,int N,int M,double *ptprice,double *ptdelta)
{
  ...
  /*  Call Price */
  CTtK=Asian_MERTON_FusaiMeucci(pseudo_stock,pseudo_strike,t,r,divid,sigma,mu,lambda,stddev,N,lowlim,uplim,M,nfft,price,solution,&delta);
...
int CALC(AP_FixedAsian_FusaiMeucciMER)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  /* Case t_0 <= T_0 */
	...
	return_value= FusaiMeucciMER_FixedAsian(pseudo_spot,pseudo_strike,ptOpt->PayOff.Val.V_NUMFUNC_2,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Lambda.Val.V_PDOUBLE,ptMod->Mean.Val.V_PDOUBLE,ptMod->Variance.Val.V_PDOUBLE,Met->Par[0].Val.V_INT2,Met->Par[1].Val.V_INT2,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE));
    }
