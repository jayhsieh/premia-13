...
/****************************************************************************/
static int optionbond_implicit1d(double maturity_option,NumFunc_1 *p,int am,
				 double t,   /*                  */
				 ...
				 double *price)
{
  ...
  for(kk=0;kk<2;kk++)
      ...
      if(kk==0)
	{
	  ...
	  dt=(maturity_bond-maturity_option)/ndt;
...
int CALC(FD_IMPLICIT_ZBO)(void *Opt,void *Mod,PricingMethod *Met)
{
  TYPEOPT* ptOpt=(TYPEOPT*)Opt;
  TYPEMOD* ptMod=(TYPEMOD*)Mod;

  return optionbond_implicit1d(ptOpt->OMaturity.Val.V_DATE,ptOpt->PayOff.Val.V_NUMFUNC_1,ptOpt->EuOrAm.Val.V_BOOL,ptMod->T.Val.V_DATE,ptOpt->BMaturity.Val.V_DATE,ptMod->alpha0.Val.V_PDOUBLE,ptMod->alphar.Val.V_PDOUBLE,ptMod->alphaf.Val.V_PDOUBLE,ptMod->gamm.Val.V_PDOUBLE,ptMod->lambda.Val.V_PDOUBLE,ptMod->beta0.Val.V_PDOUBLE,ptMod->beta1.Val.V_PDOUBLE,ptMod->eta.Val.V_PDOUBLE,ptMod->tau.Val.V_PDOUBLE,Met->Par[0].Val.V_LONG,Met->Par[1].Val.V_LONG,Met->Par[2].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE));
}


static int CHK_OPT(FD_IMPLICIT_ZBO)(void *Opt, void *Mod)
{
