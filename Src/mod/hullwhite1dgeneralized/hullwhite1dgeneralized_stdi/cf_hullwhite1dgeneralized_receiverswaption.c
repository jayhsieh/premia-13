...
///* Receiver Swaption price as a combination of ZC Put option prices
static int cf_rs1d(int flat_flag, double r_t, char *curve, int CapletCurve, double Nominal, double periodicity, double option_maturity, double contract_maturity, double SwaptionFixedRate, double a,double *price)
{
    int i, nb_payement;
    double ci, sum ,Ti;
    ...
    for(i=1; i<=nb_payement; i++)
    {
        ...
        PutOptionPrice = hw1dg_zc_call_price(&ZCMarket, &HW1dG_Parameters, Strike_i, option_maturity, Ti);
...
int CALC(CF_ReceiverSwaptionHW1dG)(void *Opt,void *Mod,PricingMethod *Met)
{
  TYPEOPT* ptOpt=(TYPEOPT*)Opt;
  TYPEMOD* ptMod=(TYPEMOD*)Mod;


  return cf_rs1d( ptMod->flat_flag.Val.V_INT,
                  MOD(GetYield)(ptMod),
                  MOD(GetCurve)(ptMod),
...
static int CHK_OPT(CF_ReceiverSwaptionHW1dG)(void *Opt, void *Mod)
{
...
static int MET(Init)(PricingMethod *Met,Option *Opt)
{
  if ( Met->init == 0)
    {
      Met->init=1;
       Met->HelpFilenameHint = "cf_hullwhite1dgeneralized_receiverswaption";
    }

  return OK;
}

PricingMethod MET(CF_ReceiverSwaptionHW1dG)=
{
  "CF_HullWhite1dG_ReceiverSwaption",
  {{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(CF_ReceiverSwaptionHW1dG),
  {{"Price",DOUBLE,{100},FORBID},{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(CF_ReceiverSwaptionHW1dG),
  CHK_ok,
  MET(Init)
} ;
