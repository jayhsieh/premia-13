...
/*number of time steps*/
	...
	{
      k++;
      alp[k]=dt*(ccp[k]+cp[k])/pow(h,alpha_plus)/a2; /* coefficients for integral c_{++}-c_+  */
      alm[k]=dt*(ccm[k]+cm[k])/pow(h,alpha_minus)/a2; /* coefficients for integral c_{--}-c_-  */
	 }
...
/*Main Loop on time grid*/
	...
	{   
		...
		confft(zreal, zimage, v1, vreal, vimage, tmp2, tmpimage, Ns, kmax, Nbin, -1);
		...
		confft(zreal, zimage, v3, vreal, vimage, tmp2, tmpimage, Ns, kmax, Nbin, -1);
...
free(zimage);
    ...
    *TimePoints=TP;
	...
	/*printf("Look 'iearly.dat' and 'iprices.dat' for results\n");
...
int CALC(FD_KLZ)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return fds_ts_amerput(
	  ...
	  ptMod->AlphaPlus.Val.V_RGDOUBLE,ptMod->AlphaMinus.Val.V_RGDOUBLE,ptMod->CPlus.Val.V_DOUBLE,ptMod->CMinus.Val.V_DOUBLE
	  ,r,divid,
