...
extern "C"{
  static int CarrTS(double S0,NumFunc_1  *p,double T,double r,double divid,double alphap,double alpham,double lambdap,double lambdam,double cp,double cm,double *ptprice,double *ptdelta)
  {       
    double K;
    double Sigma=0.2;
    ...
    TS_measure measure(alphap,alpham,lambdap,lambdam,cp,cm,0.1);
  ...
  int CALC(AP_CarrTS)(void*Opt,void *Mod,PricingMethod *Met)
  {
    ...
    return  CarrTS(ptMod->S0.Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_1,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid, ptMod->AlphaPlus.Val.V_PDOUBLE,ptMod->AlphaMinus.Val.V_PDOUBLE,ptMod->LambdaPlus.Val.V_PDOUBLE,ptMod->LambdaMinus.Val.V_PDOUBLE,ptMod->CPlus.Val.V_PDOUBLE,ptMod->CMinus.Val.V_PDOUBLE,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE));
  }
