...
extern "C"{
static int ImpExp(int am,double S0,NumFunc_1  *p,double T,double r,double divid,double alphap,double alpham,double lambdap,double lambdam,double cp,double cm,double dx,int M,int flag_scheme,double *ptprice,double *ptdelta)
{
...
int CALC(FD_ImpExp)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return ImpExp(ptOpt->EuOrAm.Val.V_BOOL,ptMod->S0.Val.V_PDOUBLE,
                ptOpt->PayOff.Val.V_NUMFUNC_1,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->AlphaPlus.Val.V_PDOUBLE,ptMod->AlphaMinus.Val.V_PDOUBLE,ptMod->LambdaPlus.Val.V_PDOUBLE,ptMod->LambdaMinus.Val.V_PDOUBLE,ptMod->CPlus.Val.V_PDOUBLE,ptMod->CMinus.Val.V_PDOUBLE,Met->Par[0].Val.V_DOUBLE,Met->Par[1].Val.V_INT,Met->Par[2].Val.V_ENUM.value,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE));
}
...
static int MET(Init)(PricingMethod *Met,Option *Opt)
{
  ...
  if (first) 
    {
      Met->HelpFilenameHint = "FD_ImpExp_ts";
