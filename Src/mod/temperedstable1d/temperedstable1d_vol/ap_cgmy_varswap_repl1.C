...
extern "C"{
  ...
  //------------------------------------------------------------------------
  static int ap_cgmy_varswap_repl1(double S0, double Strike, double T, double r, double divid, double ap, double am,double lap,double lam,double cpp,double cmm, double *fairval, double *ptprice)
  {
	...
	for(k=k0;k>=0;k--)
	{
		...
		res=iac_kobol_europut(CallPuts[k], lam, lap, am, ap, cmm, cpp, r, T, /*tstrike/*/replStrikes[k], S0*pvfactor, 0.00000001, &optprice);
		if(res) {return 1;}
  ...
  int CALC(AP_REPL1_VARIANCESWAP)(void *Opt,void *Mod,PricingMethod *Met)
  {
  ...
  static int CHK_OPT(AP_REPL1_VARIANCESWAP)(void *Opt, void *Mod)
  {
  ...
  static int MET(Init)(PricingMethod *Met,Option *Opt)
  {
    ...
    {
      first=0;
      Met->HelpFilenameHint = "ap_cgmy_varswap_repl1";
    }
    return OK;
  }

  PricingMethod MET(AP_REPL1_VARIANCESWAP)=
  {
    "AP_CGMY_VARSWAP_REPL1",
    { {" ",PREMIA_NULLTYPE,{0},FORBID}},
    CALC(AP_REPL1_VARIANCESWAP),
    {   {"Fair strike in annual volatility points",DOUBLE,{100},FORBID},
        {"Price in 10000 variance points",DOUBLE,{100},FORBID},
        {" ",PREMIA_NULLTYPE,{0},FORBID}},
    CHK_OPT(AP_REPL1_VARIANCESWAP),
    CHK_ok ,
    MET(Init)
  } ;
