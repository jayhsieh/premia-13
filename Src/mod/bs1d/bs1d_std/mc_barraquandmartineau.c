...
static void Init_Cells(int generator,NumFunc_1 *p,int BS_Dimension, int OP_Exercise_Dates, int AL_MonteCarlo_Iterations,int AL_PO_Size, double *BS_Spot,double Step, double Sqrt_Step)
{
  ...
  for (k=0;k<AL_MonteCarlo_Iterations;k++){
    ...
    auxop2=(p->Compute)(p->Par,*Path);
    auxcell2=Number_Cell(auxop2,0,AL_PO_Size);
	
    for (i=0;i<OP_Exercise_Dates-1;i++){
      auxcell1=auxcell2;
      auxop1=auxop2;
      auxop2=(p->Compute)(p->Par,*(Path+(i+1)*BS_Dimension));
...
static void InitMesh(NumFunc_1 *p,int generator,int AL_PO_Size, long Al_PO_Init, int BS_Dimension,int OP_Exercise_Dates,double *BS_Spot, double Step, double Sqrt_Step)
{
  int i,j,k,l;
  ...
  for (i=0;i<Al_PO_Init;i++){
	...
	PathAuxPO[k]= (p->Compute)(p->Par,*(PathAux+(k-1)*2*BS_Dimension+BS_Dimension));
      }
...
static void BaMa(double *AL_BPrice,long AL_MonteCarlo_Iterations,NumFunc_1 *p,int AL_PO_Size,int AL_PO_Init,int AL_ShuttingDown,int generator,int OP_Exercise_Dates,double *BS_Spot, double BS_Maturity, double BS_Interest_Rate, double divid, double sigma,int gj_flag)   
{
  ...
  *Sigma=sigma;
  Aux_BS[0]=0.5*SQR(sigma)-BS_Interest_Rate+divid;
...
static int MCBarraquandMartineau(double s, NumFunc_1  *p, double t, double r, double divid, double sigma, long N, int generator, double inc,int size,int init, int exercise_date_number,double *ptprice, double *ptdelta)
{
  ...
  if(init_mc == OK)
	...
	*ptprice=p3+7./2.*(p3-p2)-(p2-p1)/2.;
	...
	BaMa(ptprice,N,p,size,init,fermeture,generator,exercise_date_number,s_vector,t,r,divid,sigma,0);
      ...
      /*Delta*/
      if (exercise_date_number==0) {
	BaMa(&p3,N,p,size,init,fermeture,generator,4,s_vector_plus,t,r,divid,sigma,1);
	BaMa(&p2,N,p,size,init,fermeture,generator,3,s_vector_plus,t,r,divid,sigma,1);
	BaMa(&p1,N,p,size,init,fermeture,generator,2,s_vector_plus,t,r,divid,sigma,1);
	*ptdelta=((p3+7./2.*(p3-p2)-(p2-p1)/2.)-*ptprice)/(s*inc);
      } else {
	BaMa(&p1,N,p,size,init,fermeture,generator,exercise_date_number,s_vector_plus,t,r,divid,sigma,0);
...
int CALC(MC_BarraquandMartineau)(void *Opt, void *Mod, PricingMethod *Met)
{
  ...
  return MCBarraquandMartineau(ptMod->S0.Val.V_PDOUBLE,
...
static int CHK_OPT(MC_BarraquandMartineau)(void *Opt, void *Mod)
{
...
PricingMethod MET(MC_BarraquandMartineau)=
{
  "MC_BarraquandMartineau",
  ...
  CALC(MC_BarraquandMartineau),
  {{"Price",DOUBLE,{100},FORBID},
   {"Delta",DOUBLE,{100},FORBID} ,
   {" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(MC_BarraquandMartineau),
  CHK_mc,
  MET(Init)
};
