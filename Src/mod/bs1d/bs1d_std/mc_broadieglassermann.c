                 ...
                 int gj_flag) 
{
    ...
    for (i=0;i<AL_Mesh_Size;i++){
      ...
      for (k=0;k<AL_Mesh_Size;k++){
        ...
        aux+=(Price[k*OP_Exercise_Dates+j+1]+
              (p->Compute) (p->Par,*(Mesh+k*OP_Exercise_Dates*BS_Dimension+(j+1)*BS_Dimension)))*Weight(j,Mesh+i*OP_Exercise_Dates*BS_Dimension+j*BS_Dimension,Mesh+k*OP_Exercise_Dates*BS_Dimension+(j+1)*BS_Dimension,k,BS_Dimension,Step);
      }
  ...
  aux=0;
  for (i=0;i<AL_Mesh_Size;i++){
    aux+=Price[i*OP_Exercise_Dates+1]+(p->Compute) (p->Par,*(Mesh+i*OP_Exercise_Dates*BS_Dimension+BS_Dimension));
  }
  ...
  for(i=0;i<AL_MonteCarlo_Iterations;i++){  
    ...
    do {
      aux=0;
      for (k=0;k<AL_Mesh_Size;k++){
        aux+=(Price[k*OP_Exercise_Dates+j+1]+(p->Compute)(p->Par,*(Mesh+k*OP_Exercise_Dates*BS_Dimension+(j+1)*BS_Dimension)))*Weight(j,Path+j*BS_Dimension,Mesh+k*OP_Exercise_Dates*BS_Dimension+(j+1)*BS_Dimension,k,BS_Dimension,Step);
      }
      aux*=DiscountStep/(double)AL_Mesh_Size;
    ...
    AL_FPrice+=Discount((double)(j)*Step,BS_Interest_Rate)*(p->Compute) (p->Par,*(Path+(j)*BS_Dimension));
  }
...
static int MCBroadieGlassermann(double s, NumFunc_1  *p, double t, double r, double dividend, double sig, long N, int generator, double inc,int mesh_size, int exercise_date_number,double *ptprice, double *ptdelta)
{
  ...
  if(init_mc == OK)
    {
      ...
      /*Delta*/
      if (exercise_date_number==0) {
        BrGl(&p3,N,p,mesh_size,fermeture,generator,4,s_vector_plus,t,r, divid,sigma,1);
        BrGl(&p2,N,p,mesh_size,fermeture,generator,3,s_vector_plus,t,r,divid,sigma,1);
        BrGl(&p1,N,p,mesh_size,fermeture,generator,2,s_vector_plus,t,r,divid,sigma,1);
        *ptdelta=((p3+7./2.*(p3-p2)-(p2-p1)/2.)-*ptprice)/(s*inc);
      } else {
        BrGl(&p1,N,p,mesh_size,fermeture,generator,exercise_date_number,s_vector_plus,t,r,divid,sigma,0);
...
int CALC(MC_BroadieGlassermann)(void *Opt, void *Mod, PricingMethod *Met)
{
  ...
  return MCBroadieGlassermann(ptMod->S0.Val.V_PDOUBLE,
