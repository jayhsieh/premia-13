...
int CALC(DynamicHedgingSimulatorPatry5)(void *Opt,void *Mod,PricingMethod *Met,DynamicTest *Test)
{
  ...
  for (k=9;k<=24;k++)
    {
      if ((Test->Res[k].Val.V_PNLVECT->array= malloc(size*sizeof(double)))==NULL)
	return MEMORY_ALLOCATION_FAILURE;
      else
	Test->Res[k].Val.V_PNLVECT->size=size;
    }
  ...
  if (first)
    {
      ...
      Test->Par[3].Val.V_DOUBLE=0.1;    /* P&L_Target */
...
int CHK_TEST(test2)(void *Opt, void *Mod, PricingMethod *Met)
{
