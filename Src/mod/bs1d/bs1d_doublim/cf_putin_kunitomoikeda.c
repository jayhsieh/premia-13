...
static int PutIn_KunitomoIkeda_91(double s,NumFunc_1  *L,NumFunc_1  *U,NumFunc_1  *Rebate,NumFunc_1 *PayOff,
                                  double t,double r,double divid,double sigma,double *ptprice,double *ptdelta)
{
  ...
  pnl_cf_put_bs(s,PayOff->Par[0].Val.V_PDOUBLE,t,r,divid,sigma,&price,&delta);
  PutOut_KunitomoIkeda_91(s,L,U,Rebate,PayOff,t,r,divid,sigma,&out_price,&out_delta);

  /*Price*/
  *ptprice=price-out_price;
	
  pnl_cf_put_bs(s*(1.+INC),PayOff->Par[0].Val.V_PDOUBLE,t,r,divid,sigma,&price,&delta);
...
int CALC(CF_PutIn_KunitomoIkeda)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  PutIn_KunitomoIkeda_91 (ptMod->S0.Val.V_PDOUBLE,ptOpt->LowerLimit.Val.V_NUMFUNC_1, ptOpt->UpperLimit.Val.V_NUMFUNC_1,
...
static int CHK_OPT(CF_PutIn_KunitomoIkeda)(void *Opt, void *Mod)
{Option* ptOpt=(Option*)Opt;
  ...
  if ((opt->Parisian).Val.V_BOOL==WRONG)
    if((opt->RebOrNo).Val.V_BOOL==NOREBATE)
      ...
      return strcmp( ((Option*)Opt)->Name,"DoublePutInEuro");
  return WRONG;
}
...
PricingMethod MET(CF_PutIn_KunitomoIkeda)=
{
  "CF_PutIn_KunitomoIkeda",
  {{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(CF_PutIn_KunitomoIkeda),
  {{"Price",DOUBLE,{100},FORBID},{"Delta",DOUBLE,{100},FORBID} ,{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(CF_PutIn_KunitomoIkeda),
  CHK_ok,
  MET(Init)
} ;
