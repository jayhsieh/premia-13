...
static int Cryer_In(double s,NumFunc_1  *L, NumFunc_1  *U,NumFunc_1  *Rebate,NumFunc_1  *p,double t,double r,double divid,double sigma,int N,int M,double *ptprice,double *ptdelta)
{
  ...
  for (TimeIndex= 1; TimeIndex<= M; TimeIndex++)
	...
	Q[PriceIndex]=P[PriceIndex]-Z[PriceIndex];
      Q[0] += alpha*Boundary(l,p,(double)TimeIndex*k,r,divid,sigma);;
...
int CALC(FD_Cryer_In)(void*Opt,void *Mod,PricingMethod *Met)
{
...
static int CHK_OPT(FD_Cryer_In)(void *Opt, void *Mod)
{
