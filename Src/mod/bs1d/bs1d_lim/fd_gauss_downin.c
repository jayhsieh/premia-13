...
static int Gauss_DownIn(int am,double s,NumFunc_1  *p,double l,double rebate,double t,double r,double divid,double sigma,int N,int M,double theta,double *ptprice,double *ptdelta)
{
  int      Index,PriceIndex,TimeIndex;
  double   k,vv,loc,h,z,alpha,beta,gamma,y,alpha1,beta1,gamma1,down,upwind_alphacoef;
  double   *Obst,*A,*B,*C,*P,*S,price1=0.,price2,pricenh,pricen2h,priceph;
  ...
  for(TimeIndex=1;TimeIndex<=M;TimeIndex++)
    {
      /*Set Rhs*/
      price1=Boundary(l,p,TimeIndex*k,r,divid,sigma);
      S[1]=beta1*P[1]+gamma1*P[2]+alpha1*price2-alpha*price1;
      price2=price1;

      for(PriceIndex=1+1;PriceIndex<N;PriceIndex++)
...
int CALC(FD_Gauss_DownIn)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return Gauss_DownIn(ptOpt->EuOrAm.Val.V_BOOL,
...
static int CHK_OPT(FD_Gauss_DownIn)(void *Opt, void *Mod)
{
