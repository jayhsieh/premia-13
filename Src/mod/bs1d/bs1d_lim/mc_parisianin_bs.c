...
static int MC_ParisianIn(int upordown,double s,NumFunc_1 *PayOff,double l,double t,double delay,double r,double divid,double sigma,int generator,long M,int N,double increment, double confidence,double *ptprice,double *ptdelta,double *pterror_price,double *pterror_delta,double *inf_price, double *sup_price, double *inf_delta, double *sup_delta)
{
  double g, h;
	  ...
	  while (((inside) && (k<N)) ||((inside_increment) && (k<N)))
	    {
	      ...
	      if (upordown==0)
		check_parisianin(&gt,lnspot,lastlnspot,barrier,lastbarrier,
				       &gt_increment,lnspot_increment,lastlnspot_increment,
				       rap,upordown,h,time,&correction_active,generator);
	      else 
		check_parisianin(&gt_increment,lnspot_increment,lastlnspot_increment,barrier,lastbarrier,&gt,lnspot,lastlnspot,rap,upordown,h,time,&correction_active,generator);
	      ...
	      if(hd>delay) 
		{
		  inside=0;
		  if (time>t) time=t;
		  price_sample=exp(-r*time)*Boundary(exp(lnspot),PayOff,t-time,r,divid,sigma);
		}
	      ...
	      if(hd_increment>delay) 
		{
		  inside_increment=0;
		  if (time>t) time=t;
		  price_sample_increment=exp(-r*time)*Boundary(exp(lnspot_increment),PayOff,t-time,r,divid,sigma);
		}
...
int CALC(MC_ParisianIn)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return MC_ParisianIn(upordown,
		       ...
		       Met->Par[2].Val.V_INT,Met->Par[3].Val.V_PDOUBLE,
...
static int MET(Init)(PricingMethod *Met,Option *Opt)
{
  ...
  if ( Met->init == 0)
    {
      Met->init=1;
      Met->HelpFilenameHint = "MC_Parisianin_bs";
