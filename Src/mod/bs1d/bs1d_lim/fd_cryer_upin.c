...
static int Cryer_UpIn(int am,double s,NumFunc_1  *p,double l,double rebate,double t,double r,double divid,double sigma,int N,int M,double *ptprice,double *ptdelta)
{
  int      Index,PriceIndex,TimeIndex,ssl;
  double   k,vv,loc,h,z,alpha,beta,gamma,y,up,upwind_alphacoef,price1;
  double   *Obst,*A,*B,*C,*P,*S,*Z,*Q,pricenh,pricep2h,priceph;
  ...
  for (PriceIndex = 1; PriceIndex < N; PriceIndex++)
    Obst[PriceIndex - 1]=(p->Compute)(p->Par,exp(y-loc+PriceIndex* h));
  ...
  for (TimeIndex= 1; TimeIndex<= M; TimeIndex++)
	...
	Q[PriceIndex]=P[PriceIndex]-Z[PriceIndex];
      ...
      Q[0] += alpha*(p->Compute)(p->Par,exp(y-loc));
      Q[N-2]+=gamma*price1;
  ...
  Index=(int)floor(loc/h)-1;

  *ptprice=P[Index]+(P[Index+1]-P[Index])*(exp(y)-exp(y-loc+h+Index*h))/(exp(y-loc+h+(Index+1)*h)-exp(y-loc+h+Index*h));

  /*Delta*/
  priceph=P[Index-1]+(P[Index]-P[Index-1])*(exp(y-h)-exp(y-loc+h+(Index-1)*h))/(exp(y-loc+h+(Index)*h)-exp(y-loc+h+(Index-1)*h));
  if (y!=up) {
    pricenh=P[Index+1]+(P[Index+2]-P[Index+1])*(exp(y+h)-exp(y-loc+h+(Index+1)*h))/(exp(y-loc+h+(Index+2)*h)-exp(y-loc+h+(Index+1)*h));
    *ptdelta=(pricenh-priceph)/(2*s*h);
  } else {
    pricep2h=P[Index-2]+(P[Index-3]-P[Index-2])*(exp(y-2*h)-exp(y-loc+h+(Index-2)*h))/(exp(y-loc+h+(Index-3)*h)-exp(y-loc+h+(Index-2)*h));
...
int CALC(FD_Cryer_UpIn)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return Cryer_UpIn(ptOpt->EuOrAm.Val.V_BOOL,ptMod->S0.Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_1,
...
static int CHK_OPT(FD_Cryer_UpIn)(void *Opt, void *Mod)
{
  Option* ptOpt=(Option*)Opt;
  TYPEOPT* opt=(TYPEOPT*)(ptOpt->TypeOpt);

  if ((opt->Parisian).Val.V_BOOL==WRONG)
    if ( (strcmp( ((Option*)Opt)->Name,"CallUpInAmer")==0) || (strcmp( ((Option*)Opt)->Name,"PutUpInAmer")==0) )
      return OK;
...
PricingMethod MET(FD_Cryer_UpIn)=
{
  "FD_Cryer_UpIn",
