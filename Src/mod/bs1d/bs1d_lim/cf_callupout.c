...
static int CallUpOut_ReinerRubinstein(double s,double k,double l,double rebate,double t,double r,double divid,double sigma,double *ptprice,double *ptdelta)
{
  int phi,eta;
  double A,B,C,D,E,F;
...
int CALC(CF_CallUpOut)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return CallUpOut_ReinerRubinstein(ptMod->S0.Val.V_PDOUBLE,(ptOpt->PayOff.Val.V_NUMFUNC_1)->Par[0].Val.V_PDOUBLE,
...
static int CHK_OPT(CF_CallUpOut)(void *Opt, void *Mod)
{Option* ptOpt=(Option*)Opt;
  TYPEOPT* opt=(TYPEOPT*)(ptOpt->TypeOpt);
	 
  if ((opt->Parisian).Val.V_BOOL==WRONG)
    return strcmp( ((Option*)Opt)->Name,"CallUpOutEuro");
  return WRONG;
...
PricingMethod MET(CF_CallUpOut)=
{
  "CF_CallUpOut",
  {{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(CF_CallUpOut),
  {{"Price",DOUBLE,{100},FORBID},{"Delta",DOUBLE,{100},FORBID} ,{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(CF_CallUpOut),
  CHK_ok,
  MET(Init)
} ;
