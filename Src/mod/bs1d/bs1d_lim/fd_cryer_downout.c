...
static int Cryer_DownOut(double s,NumFunc_1  *p,double l,double rebate,double t,double r,double divid,double sigma,int N,int M,double *ptprice,double *ptdelta)
{
  int      Index,PriceIndex,TimeIndex,ssl;
  double   k,vv,loc,h,z,alpha,beta,gamma,y,down,upwind_alphacoef;
...
int CALC(FD_Cryer_DownOut)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return Cryer_DownOut(ptMod->S0.Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_1,
...
static int CHK_OPT(FD_Cryer_DownOut)(void *Opt, void *Mod)
{
  Option* ptOpt=(Option*)Opt;
  TYPEOPT* opt=(TYPEOPT*)(ptOpt->TypeOpt);

  if ((opt->Parisian).Val.V_BOOL==WRONG)
    if ( (strcmp( ((Option*)Opt)->Name,"CallDownOutAmer")==0) || (strcmp( ((Option*)Opt)->Name,"PutDownOutAmer")==0) )
      return OK;
...
PricingMethod MET(FD_Cryer_DownOut)=
{
  "FD_Cryer_DownOut",
  {{"SpaceStepNumber",INT2,{100},ALLOW    },{"TimeStepNumber",INT2,{100},ALLOW},
   {" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(FD_Cryer_DownOut),
  {{"Price",DOUBLE,{100},FORBID},{"Delta",DOUBLE,{100},FORBID} ,{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(FD_Cryer_DownOut),
  CHK_split,
  MET(Init)
};
