...
static int Psor_UpIn(double s,NumFunc_1  *p,double l,double rebate,double t,double r,double divid,double sigma,int N,int M,double theta,double omega,double epsilon,double *ptprice,double *ptdelta)
{
  ...
  for(TimeIndex=1;TimeIndex<=M;TimeIndex++)
    {
      ...
      P[N+1]=Boundary(l,p,(double)TimeIndex*k,r,divid,sigma);
  ...
  *ptprice=P[Index]+(P[Index+1]-P[Index])*(exp(x)-exp(x-loc+Index*h))/(exp(x-loc+(Index+1)*h)-exp(x-loc+Index*h));
  ...
  /*Delta*/
  priceph=P[Index-1]+(P[Index]-P[Index-1])*(exp(x-h)-exp(x-loc+(Index-1)*h))/(exp(x-loc+(Index)*h)-exp(x-loc+(Index-1)*h));
  if (x!=up) {
    pricenh=P[Index+1]+(P[Index+2]-P[Index+1])*(exp(x+h)-exp(x-loc+(Index+1)*h))/(exp(x-loc+(Index+2)*h)-exp(x-loc+(Index+1)*h));
    *ptdelta=(pricenh-priceph)/(2*s*h);
  } else {
    pricep2h=P[Index-2]+(P[Index-3]-P[Index-2])*(exp(x-2*h)-exp(x-loc+(Index-2)*h))/(exp(x-loc+(Index-3)*h)-exp(x-loc+(Index-2)*h));
...
int CALC(FD_Psor_UpIn)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return Psor_UpIn(ptMod->S0.Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_1,limit,rebate,
		   ...
		   Met->Par[0].Val.V_INT,Met->Par[1].Val.V_INT, Met->Par[2].Val.V_RGDOUBLE051,Met->Par[3].Val.V_RGDOUBLE12,Met->Par[4].Val.V_RGDOUBLE,
		   &(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE));
}

static int CHK_OPT(FD_Psor_UpIn)(void *Opt, void *Mod)
{
