...
static int  FixedAsian_Stratification(double s, double K, double time_spent, NumFunc_2 *p, double t, double r, double divid, double sigma, long nb,int nb_strat, int M, int generator, double confidence, double *ptprice,double *ptdelta, double *pterror_price, double *pterror_delta, double *inf_price, double *sup_price, double *inf_delta, double *sup_delta)
{
  ...
  if(init_mc == OK)
	...
	{
	  mean_price_strata= 0.0;
	  mean_delta_strata= 0.0;
	  var_price_strata= 0.0;
	  var_delta_strata= 0.0;
	  ...
	  for(ipath= 1;ipath<= nb;ipath++)
	      ...
	      for(i=0 ; i< step_number ; i++) 
		{
	      ...
	      /*Simulation of Stock and Average*/
	      integral=0.0;
	      S_t=s;
	      for(i=0 ; i< step_number ; i++) 
		{
		...
		delta_sample=(1-time_spent)*(integral/(s*(double)step_number))*exp(-dot1-0.5*dot2);
	      ...
	      /* Sum */
	      mean_price_strata+= price_sample;
	      mean_delta_strata+= delta_sample;

	      /* Sum of squares */
	      var_price_strata+= SQR(price_sample);
	      var_delta_strata+= SQR(delta_sample);

	    }/*End of MonteCarlo*/

	  var_price_strata/=nb;
	  var_delta_strata/=nb;
	  mean_price_strata/=nb;
	  mean_delta_strata/=nb;

	  var_price_strata-= SQR(mean_price_strata);
	  var_delta_strata-= SQR(mean_delta_strata);
	  
	  /* Sum */
	  mean_price+= mean_price_strata;
	  mean_delta+= mean_delta_strata;

	  /* Sum of squares */
	  var_price+=var_price_strata;
	  var_delta+=var_delta_strata;
      ...
      *pterror_price= exp(-r*t)*sqrt(var_price/(double)(nb*SQR(nb_strat)));
	...
	*ptdelta *= (-1); 
      *pterror_delta= sqrt(exp(-2.0*r*t)*(var_delta/((double)nb*SQR(nb_strat))));
...
int CALC(MC_FixedAsian_Stratification)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  /* Case t_0 <= T_0 */
	...
	return_value= FixedAsian_Stratification(pseudo_spot,
						pseudo_strike,
						time_spent,
						ptOpt->PayOff.Val.V_NUMFUNC_2,
						T-T_0,
...
static int CHK_OPT(MC_FixedAsian_Stratification)(void *Opt, void *Mod)
{
...
PricingMethod MET(MC_FixedAsian_Stratification)=
{
  "MC_FixedAsian_Stratification_GHS",
  ...
  CALC(MC_FixedAsian_Stratification),
  {{"Price",DOUBLE,{100},FORBID},
   ...
   {" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(MC_FixedAsian_Stratification),
  CHK_ok,
  MET(Init)
};
