...
/*/////////////////////////////////////////////////////*/
static int wh_rstemperedstable_amerdigital(double Spot, 
			      double T, double h, double Strike1,
			       double rebate,
			      double er, long int step, int n_state,
			      double *ptprice, double *ptdelta)
{
	PnlVect *divi, *rr, *lambdap, *num, *nup, *lambdam, *cm, *cp, *strike,  *rebates, *mu, *qu;
    ...
    T, h, strike, Strike1, rebates, er, step, eps, prices, deltas);
...
//===================================================================================
int CALC(AP_fastwhamerdig_rstemperedstable)(void *Opt,void *Mod,PricingMethod *Met)
{
   TYPEOPT* ptOpt=( TYPEOPT*)Opt;
  TYPEMOD* ptMod=( TYPEMOD*)Mod;
  double strike, spot,rebate;
  ...
  res = wh_rstemperedstable_amerdigital(spot,
...
static int CHK_OPT(AP_fastwhamerdig_rstemperedstable)(void *Opt, void *Mod)
{
...
PricingMethod MET(AP_fastwhamerdig_rstemperedstable)=
{
  "AP_FastWHDig_RSTS",
  ...
  CALC(AP_fastwhamerdig_rstemperedstable),
  {{"Price of chosen state",DOUBLE,{100},FORBID},
   {"Delta of chosen state",DOUBLE,{100},FORBID},
   {" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(AP_fastwhamerdig_rstemperedstable),
  CHK_split,
  MET(Init)
};
