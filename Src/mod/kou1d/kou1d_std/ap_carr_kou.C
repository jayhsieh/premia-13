...
extern "C"{
  static int CarrKou(double S0,NumFunc_1  *p,double T,double r,double divid,double sigma,double lambda,double lambdap,double lambdam,double P,double *ptprice,double *ptdelta)
  {       
    double K;
    double Sigma=0.2;
    ...
    Kou_measure measure(lambda,lambdap,lambdam,P,sigma,0.1);
    ...
    std::vector<double> z(Nlimit), z_img(Nlimit),y(Nlimit), y_img(Nlimit);
  ...
  int CALC(AP_CarrKou)(void*Opt,void *Mod,PricingMethod *Met)
  {
    ...
    return  CarrKou(ptMod->S0.Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_1,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Lambda.Val.V_PDOUBLE,ptMod->LambdaPlus.Val.V_PDOUBLE,ptMod->LambdaMinus.Val.V_PDOUBLE,ptMod->P.Val.V_PDOUBLE,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE));
  }

static int CHK_OPT(AP_CarrKou)(void *Opt, void *Mod)
  {
