...
static int fft_kou_lookbackfixed(double s_maxmin,NumFunc_2*P,double S0,double T,double r,double divid,double sigma,double lambda,double lambdam,double lambdap,double p,int n_points,long M,double *ptprice)
{ 
...
//CALL
  if ((P->Compute)==&Call_OverSpot2)
  {
    ...
    res= CRsub(CRmul(res,exp(beta*sigma*sqrt(T/n_points))*exp(-r*T)*S0),K*exp(-r*T));
  }
//PUT
  if ((P->Compute)==&Put_OverSpot2)
  {
    ...
    x_maxmin=signe*log ((s_maxmin/S0));
    res=estiation_kou(x_maxmin,G,pas,sigma,drift,lambda,lambdap,lambdam,p,h,signe,M,n_points, signe);
    res= CRadd(CRmul(res,-S0*exp(-beta*sigma*sqrt(T/n_points))*exp(-r*T)),K*exp(-r*T));
  }
  *ptprice=Creal(res);
...
int CALC(FFT_Kou_FixedLookback)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  fft_kou_lookbackfixed((ptOpt->PathDep.Val.V_NUMFUNC_2)->Par[4].Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Lambda.Val.V_PDOUBLE,ptMod->LambdaPlus.Val.V_PDOUBLE,ptMod->LambdaMinus.Val.V_PDOUBLE,ptMod->P.Val.V_PDOUBLE,Met->Par[0].Val.V_PINT,Met->Par[1].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE));
}
