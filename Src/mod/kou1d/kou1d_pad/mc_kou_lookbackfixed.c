...
static int Kou_Mc_Fixed(double s_maxmin,NumFunc_2*P,double S0,double T,double r,double divid,double sigma,double lambda,double lambdap,double lambdam,double p,int generator,int n_points,long n_paths,double *ptprice,double *ptdelta,double *priceerror,double *deltaerror)
{
  double *s,s1,s2,*s3,**s4,s5,s6;
  double pas,sup,inf,supw,infw,exp_sup,exp_inf,exp_supw,exp_infw,beta;
  double *t,*jump_size_vect,*jump_time_vect,sup0,temp,nu,discount,*cov_payoff_control;
  double var_payoff,**var_control,cor_payoff_control,*coef_control,var_proba,determinant_control;
  double *X,*W,payoff,*control,proba,s0,u0,s_shift,e1,e2,e3,expect_control_0,K;
...
int CALC(MC_Kou_Fixed)(void*Opt,void *Mod,PricingMethod *Met)
{
  ...
  return  Kou_Mc_Fixed((ptOpt->PathDep.Val.V_NUMFUNC_2)->Par[4].Val.V_PDOUBLE,ptOpt->PayOff.Val.V_NUMFUNC_2,ptMod->S0.Val.V_PDOUBLE,ptOpt->Maturity.Val.V_DATE-ptMod->T.Val.V_DATE,r,divid,ptMod->Sigma.Val.V_PDOUBLE,ptMod->Lambda.Val.V_PDOUBLE,ptMod->LambdaPlus.Val.V_PDOUBLE,ptMod->LambdaMinus.Val.V_PDOUBLE,ptMod->P.Val.V_PDOUBLE,Met->Par[0].Val.V_ENUM.value,Met->Par[1].Val.V_PINT,Met->Par[2].Val.V_LONG,&(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE),&(Met->Res[2].Val.V_DOUBLE),&(Met->Res[3].Val.V_DOUBLE));
}

static int CHK_OPT(MC_Kou_Fixed)(void *Opt, void *Mod)
{
...
PricingMethod MET(MC_Kou_Fixed)=
{
  "MC_Kou_LookbackFixed",
