...
static void Simul_StockAndAverage_RobbinsMonro(int generator, int step_number, double T,  double x, double r, double divid, double sigma0,double nu,double sigma2,double rho, NumFunc_1 *p)
{
  int RM=5000;
  int sig_itere=0;
  double    S_t,  g1, g2,K;
  ...
  double trend= nu- 0.5 * SQR(sigma2);
  ...
  for(ii=0;ii<RM;ii++) {
    ...
    for(i=0 ; i< step_number ; i++) {
      ...
      V_t=V_t*exp(trend*h+sigma2*sqrt_h*value);
      V_t=MIN(V_t,2.0);
...
static int MCRobbinsMonro(double s, NumFunc_1  *p, double t, double r, double divid, double sigma0,double nu,double sigma2,double rho, long nb, int M,int generator, double confidence, double *ptprice, double *ptdelta, double *pterror_price, double *pterror_delta , double *inf_price, double *sup_price, double *inf_delta, double *sup_delta)
{
  long i,ipath;
  ...
  Normalvect= malloc(sizeof(double)*(2*(nb*step_number+1)));
  ...
  if(init_mc == OK)
    {
      ...
      /* Price  */
      (void)Simul_StockAndAverage_RobbinsMonro(generator, M, t, s,r, divid, sigma0,nu,sigma2,rho, p);
      ...
      for(ipath= 1;ipath<= nb;ipath++)
	{
	  ...
	  for(i=0 ; i<step_number ; i++) {
	    ...
	    g2= pnl_rand_gauss(step_number, RETRIEVE, (2*i)+1, generator);
	    Normalvect[i+(ipath-1+nb)*step_number]=g2;
	  ...
	  dot1=0.;
	  for(i=0;i<step_number;i++){
	    dot1+=m_Mu[i]*Normalvect[i+(ipath-1)*step_number]+m_Mu[i+step_number]*Normalvect[i+(ipath-1+nb)*step_number];
	  }
...
int CALC(MC_RobbinsMonro_HullWhite)(void *Opt, void *Mod, PricingMethod *Met)
{
...
static int CHK_OPT(MC_RobbinsMonro_HullWhite)(void *Opt, void *Mod)
{
...
PricingMethod MET(MC_RobbinsMonro_HullWhite)=
{
  "MC_RobbinsMoro_HW",
  ...
  CALC(MC_RobbinsMonro_HullWhite),
  {{"Price",DOUBLE,{100},FORBID},
   ...
   {" ",PREMIA_NULLTYPE,{0},FORBID}},
  CHK_OPT(MC_RobbinsMonro_HullWhite),
  CHK_mc,
  MET(Init)
};
