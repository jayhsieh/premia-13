...
static int GMRES(int am,double s1,double s2,NumFunc_2  *p,double t,double r,double divid1,double divid2,double sigma1,double sigma2,double rho,int N, int M,int max_iter,double tol,int m,int precond,double *ptprice,double *ptdelta1,double *ptdelta2)
{
  ...
  for (i=0;i<10;i++)
    {
      band[i]=(double *)calloc(Ns+1,sizeof(double));
      if (band[i]==NULL)
	return MEMORY_ALLOCATION_FAILURE;
    }
  ...
  /*Dirichlet Boundary Conditions*/ 
  Dirichlet(N,a2,b2,c2,d2,e2,f2,g2,i2,j2,x1,x2,limit1,limit2,h1,h2,
	    p,bound);
  ...
  for (TimeIndex=1;TimeIndex<=M;TimeIndex++) 
      ...
      for (i=0;i<m+1;i++)
	{
      ...
      /*GMRES Algorithm*/
      gmres(H,band,P,b,m,precond,Ns,max_iter,tol,pivots);
...
int CALC(FD_GMRES)(void *Opt,void *Mod,PricingMethod *Met)
{
  ...
  return GMRES(ptOpt->EuOrAm.Val.V_BOOL,ptMod->S01.Val.V_PDOUBLE,
	       ...
	       Met->Par[0].Val.V_INT,Met->Par[1].Val.V_INT,Met->Par[2].Val.V_INT,Met->Par[3].Val.V_PDOUBLE,Met->Par[4].Val.V_INT,Met->Par[5].Val.V_ENUM.value,
	       &(Met->Res[0].Val.V_DOUBLE),&(Met->Res[1].Val.V_DOUBLE),&(Met->Res[2].Val.V_DOUBLE) );
}

static int CHK_OPT(FD_GMRES)(void *Opt, void *Mod)
{
  return  OK;
}
  ...
  if ( Met->init == 0)
    {
      ...
      Met->Par[5].Val.V_ENUM.members=&PremiaEnumPrecond;
...
PricingMethod MET(FD_GMRES)=
{
  ...
  {{"SpaceStepNumber",INT2,{100},ALLOW},{"TimeStepNumber",INT2,{100},ALLOW}
   ,{"Max Iter",INT2,{100},ALLOW},{"Tol",PDOUBLE,{100},ALLOW},{"Restart Number",INT,{100},ALLOW},{"Precondtioner",ENUM,{100},ALLOW},{" ",PREMIA_NULLTYPE,{0},FORBID}},
  CALC(FD_GMRES),
