...
static void CapFloor_InitialPayoffBK1D(TreeShortRate* Meth, ModelParameters* ModelParam, PnlVect* ZCbondPriceVect, PnlVect* OptionPriceVect, int i_T,  NumFunc_1 *p, double periodicity, double CapFloorFixedRate)
{
    ...
    BackwardIteration(Meth, ModelParam, OptionPriceVect, ZCbondPriceVect, Meth->Ngrid, i_T, &func_model_bk1d);
...
// Dynamic programming + adding payoff at each reseting date
static void CapFloor_BackwardIterationBK1D(TreeShortRate* Meth, ModelParameters* ModelParam, NumFunc_1 *p, PnlVect* ZCbondPriceVect1, PnlVect* ZCbondPriceVect2, PnlVect* OptionPriceVect1, PnlVect* OptionPriceVect2, int index_last, int index_first, double periodicity, double CapFloorFixedRate)
{
    double a ,sigma;
...
static double tr_bk1d_capfloor(TreeShortRate* Meth, ModelParameters* ModelParam, ZCMarketData* ZCMarket, int NumberOfTimeStep, NumFunc_1 *p, double r, double periodicity,double first_reset_date,double contract_maturity, double CapFloorFixedRate)
{
    ...
    for(i = n-2; i>=0; i--)
    {
        ...
        CapFloor_BackwardIterationBK1D(Meth, ModelParam,p, ZCbondPriceVect1, ZCbondPriceVect2, OptionPriceVect1, OptionPriceVect2,  i_Ti2, i_Ti1, periodicity, CapFloorFixedRate);
    }
...
static int tr_capfloor1d(int flat_flag, double r0,char *curve,double a,double sigma, double contract_maturity, double first_reset_date, double periodicity,double Nominal, double CapFloorFixedRate, NumFunc_1 *p, long N_steps, double *price)
{
    ...
    /* Flag to decide to read or not ZC bond data in "initialyields.dat" */
    ...
    *price =  Nominal * tr_bk1d_capfloor(&Tr, &ModelParams, &ZCMarket, N_steps, p, r0, periodicity, first_reset_date, contract_maturity, CapFloorFixedRate);
