...
static void Valcf (int N, PnlVect *omega, double u0, double u, double r,
                   double q, double T, double eta, double rho, double
                   lambda, double x, double a, PnlVectComplex *cf)
{
  int j;
  ...
  for (j=0;j<N;j++)
    {
      ...
      D=Cpow_real(Cadd(Cpow_real(RCsub(lambda,Complex(0,rho*eta*omegaj)),2),CRmul(Complex(pow(omegaj,2),omegaj),pow(eta,2))),0.5);
      G=Cdiv(Csub(Complex(lambda,-rho*eta*omegaj),D),Cadd(Complex(lambda,-rho*eta*omegaj),D));     

      temp1=RCsub(1,Cexp(CRmul(D,-T)));
      temp2=RCsub(1,Cmul(G,Cexp(CRmul(D,-T))));
      temp3=Csub(Complex(lambda,-rho*eta*omegaj),D);
      temp4=RCsub(1,G);

      pnl_vect_complex_set(cf,j,Cmul(Cmul(Cexp(Cadd(Complex(0,omegaj*(r-q)*T),CRmul(Cmul(Cdiv(temp1,temp2), temp3),u0/pow(eta,2)))), Cexp(RCmul(lambda*u/pow(eta,2), Csub(CRmul(temp3,T),RCmul(2,Clog(Cdiv(temp2,temp4))))))), Cexp(Complex(0,(x-a)*omegaj))));
...
static int Cosine(double S0, double K, double T, double r, double q, double
                  u0, double u, double lambda, double eta, double rho, int
                  iscall, double *prix)
{
  ...
  /*Cumulants*/
  c1=(r-q)*T+(1-exp(-lambda*T))*(u-u0)/(2.0*lambda)-0.5*u*T;
  c2=(1.0/(8.0*pow(lambda,3)))*(eta*T*lambda*exp(-lambda*T)*(u0-u)*(8.0*lambda*rho-4.0*eta)+lambda*rho*eta*(1.0-exp(-lambda*T))*(16.0*u-8.0*u0)+2.0*u*lambda*T*(-4.0*lambda*rho*eta+pow(eta,2)+4*pow(lambda,2))+pow(eta,2)*((u-2.0*u0)*exp(-2*lambda*T)+u*(6.0*exp(-lambda*T)-7)+2*u0)+8*pow(lambda,2)*(u0-u)*(1-exp(-lambda*T)));

  /*Truncation range*/
  a=c1-L*pow(fabs(c2),0.5)+x;
  b=c1+L*pow(fabs(c2),0.5)+x;

  Valomega(N, a, b, omega);
	
  /*Characteristic function of Heston model*/
  Valcf(N, omega, u0, u, r, q, T, eta, rho, lambda, x, a, cf); 
  cf0(cf);
  ...
  *prix =  pnl_vect_sum (fcvec);
  ...
  /* The value of a call option is obtained from that of a put option, by
   * put-call parity */
  ...
  if ( Met->init == 0 )
    {
      ...
      Met->init = 1;
      Met->HelpFilenameHint = "ap_cosine_hes1d_euro";
    }
  return OK;
}
