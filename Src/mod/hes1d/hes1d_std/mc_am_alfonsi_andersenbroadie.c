    ...
    for (m=0; m<NbrMCsimulationDual; m++)
    {
        ...
        for (i=1; i<=NbrExerciseDates-2; i++)
        {
            ...
            if (ExerciceOrContinuation)
            {
                ...
                HestonSimulation_Alfonsi(flag_SpotPaths, SpotPaths_inner, flag_VarPaths, VarPaths_inner, flag_AveragePaths, AveragePaths, S_t, step, r, divid, V_t, k, theta, sigma, rho, NbrMCsimulationDualInternal, 2, NbrStepPerPeriod, generator, flag_cir);
