...
int main()
{
  ...
  double delta = .25; // tenor equal to 3 months
  double L_0 = .1;// initial flat value
  double theta = .12; // strike
  float swaptionMat=1.0;        //(years)
  float swapMat=10.0;            //(years)
    
  int k = (int)(swapMat/delta + .5);//last exercise date 
  int first = (int)(swaptionMat/delta + .5);//first exercise date

 /* cf article */
  int d = 10;//new dimension after the reduction by principal component analysis
  double a = 1.5, b = 3.5, c = .2, ginf = .5, phi = 0.0413;;
  int p = 1; //precision for the log_Euler Scheme (precision increases with p since dt = delta/p)
  
  int choice;   
  cout << "Bermudan swaption's pricing by Julien Bourgouint, July 2006." << endl << "  0. upper & lower bound using fast approach" << endl << endl << "  Tight Lower bound" << endl << "  1. Andersen Algorithm" << endl << "  2. Kolodko & Schoenmakers Algorithm" << endl << "  3. one step Kolodko & Schoenmakers' improvement after Andersen Algorithm" << endl << endl << "  Tight Upper bound" << endl << "  4. with Andersen Algorithm" << endl << "  5. with Kolodko & Schoenmakers Algorithm" << endl << endl << "remarks : please be careful with this refined version. Precise results need long time computations" << endl;
  ...
  switch(choice)
    ...
    case 0:
      {
	LMM Lib(first, k, L_0,  d, phi, a, b, c, ginf, delta, p, theta);
	Lib.InitialCond();
	Lib.print_CF();
	...
	cin >> N;
	LMM Lib2(first, k, L_0,  d, phi, a, b, c, ginf, delta, p, theta);
	Lib2.InitialCond();
	Lib2.print_MC(N); 
	...
	cin >> Nup;
	LMM Libup(first, k, L_0, d, phi, a, b, c, ginf, delta, p, theta);
	...
	Andersen Ander(strat, first, k, L_0, d, phi, a, b, c, ginf, delta, p, theta, div, NH, NS);
	Ander.estimate();
	break;
      }  
    case 2:
	...
	cin >> N;
	Y1 y1(first, k, L_0, d, phi, a, b, c, ginf, delta, p, theta, N);
	y1.estimate();
	...
	cin >> N2;
	Y2 y2(first, k, L_0, d, phi, a, b, c, ginf, delta, p, theta, N1, N2);
	y2.estimate();
	break;
      }
    case 3 :
	...
	int N1;// for the price estimator after 2 iterations
	cout << "Number of Monte Carlo Iterations for the price estimator "<<endl;
	cin >> N1;
	...
	Andersenhat Anderhat(N1, N2, strat, first, k, L_0, d, phi, a, b, c, ginf, delta, p, theta, div, NH);
	Anderhat.estimate2();
	break;
      }
    case 4 :
	...
	YAup yaup(strat, first, k, L_0, d, phi, a, b, c, ginf, delta, p, theta, div, NH, NKA, N1upA, N2upA);
	yaup.estimate2();
	break;
      }   
    case 5 :
	...
	Y1up y1up(first, k, L_0, d, phi, a, b, c, ginf, delta, p, theta, NK, N1up, N2up);
	y1up.estimate();
	break;
      }
    default: 
