...
LMM::LMM(int _first, int _k, double _L_0,  double _sigma, double _delta, int _p, double _theta):first(_first), kinit(_k), sigma(_sigma), delta(_delta), p(_p), theta(_theta), dt(delta/p), L_0(_L_0), sqrt_dt(sqrt(dt))
...
Y1::Y1(int first, int k, double L_0, double sigma, double delta, int p, double theta, int _N):LMM(first, k, L_0, sigma, delta, p, theta), N(_N)
{
};

Y2::Y2 (int first, int k, double L_0, double sigma, double delta, int p, double theta, int _N1, int _N2):LMM(first, k, L_0, sigma, delta, p, theta), N1(_N1), N2(_N2)
{
  P.resize(k);
  Lmem.resize(k);
};

Andersen::Andersen(int _strat, int first, int k, double L_0, double sigma, double delta, int p, double theta, int _div, int _NH, int _NS):strat(_strat), LMM(first, k, L_0, sigma, delta, p, theta), div(_div), NH(_NH), NS(_NS)
{  
  H.resize(k); 
  H = .1; // H is initialized with a high value (>> payoff)
  H[k-1] = 0.;
};


Andersenhat::Andersenhat(int _N1, int _N2, int strat, int first, int k, double L_0, double sigma, double delta, int p, double theta, int div, int NH):N1(_N1),N2(_N2), Andersen(strat, first, k, L_0, sigma, delta, p, theta, div, NH, 0)
{
  P.resize(k);
  Lmem.resize(k);
};

Y1up::Y1up(int first, int k, double L_0, double sigma, double delta, int p, double theta, int _NK, int _N1up, int _N2up):LMM(first, k, L_0, sigma, delta, p, theta), NK(_NK), N1up(_N1up), N2up(_N2up)
{
  Lmem1.resize(k);
  Lmem2.resize(k);
};

YAup::YAup(int strat, int first, int k, double L_0, double sigma, double delta, int p, double theta, int div, int NH, int _NK, int _N1up, int _N2up):Andersen(strat, first, k, L_0, sigma, delta, p, theta, div, NH, 0), NK(_NK), N1up(_N1up), N2up(_N2up)
{
  Lmem1.resize(k);
  Lmem2.resize(k);
};
...
{ 
  ...
  for (l = 0; l < p; l++) //time path
	...
	{ 
	  ...
	  det *= sigma*sigma*delta;
	  L[i] *= 1 + det*dt + sigma*dw;
	}
    } 
  /* payoff computation */ 
  payoff = 0.;
...
*/
{
  int p;
  double Bpq = 0.;
  double Spq, dplus, dmoins, resultat;
  double maxi = 0.;
  for (p = k-1; p >= 1; p--)
    { 
      Bpq += delta*B[p+1]; 
      Spq = (B[p] - B[k])/Bpq; 
      dplus = log(Spq/theta)/(sigma*sqrt(p*delta)) + sigma*sqrt(p*delta)/2;
      dmoins = dplus - sigma*sqrt(p*delta);
      resultat = Bpq*(Spq*Cumul_Normal(dplus) - theta*Cumul_Normal(dmoins));
...
*/
{
  ...
  double Bpq = 0.;
  double Spq, dplus, dmoins, resultat;
  for (p = k-1; p >= 1; p--) Bpq += B[p+1];
  Bpq *= delta;
  Spq = (B[1] - B[k])/Bpq; 
  dplus = log(Spq/theta)/(sigma*sqrt(p*delta)) + sigma*sqrt(p*delta)/2;
  dmoins = dplus - sigma*sqrt(p*delta);
  resultat = Bpq*(Spq*Cumul_Normal(dplus) - theta*Cumul_Normal(dmoins));
  return (resultat*actual[kinit-k]);
}
...
*/
{
  ...
  for (p = k; p >= first; p--)
    { 
      ...
      dplus = log(Spq/theta)/(sigma*sqrt(p*delta)) + sigma*sqrt(p*delta)/2;
      dmoins = dplus - sigma*sqrt(p*delta);
      tab[p-1] = Bpq*(Spq*Cumul_Normal(dplus) - theta*Cumul_Normal(dmoins));
    }
  tab *= 10000; //result in base points
...
/* rough upper bound estimation (provide arithmetic mean, standard deviation and a 95% confidence interval) */
{
  ...
  inter *= 10000; mean *= 10000;//result in base points
...
/* builds a table with Z_tau1 values */
{
  ...
  bool A[k-1]; // A[i] false means exercice at time i. table A length is k-1 because we exercise at time k in all cases.
  for(i = 0; i < kref; i++)
    {    
      Scheme();
      P[i] = payoff; 
...
/* finds the optimal frontier exercice using golden section */
{
  tab_payoff(NH);
  double h;
  double ax = 0.;
...
/* builds a table of the payoffs with Andersen exercise strategy */
{
  ...
  int kref = k;
  for(int i = 0; i < k; i++) Lmem[i] = L[i];
  bool A[k-1]; // A[i] false = exercise at time i.
  for(i = 0; i < kref; i++)
    {    
      Scheme();
      P[i] = payoff; 
