...
double calibration_data_QuadraticError(const Calibration_Data * data)
{
  ...
  list_option_eqd_compute_implied_vol(opmodel);
  for(i=0; i<opmarket->nb_options; i++)
  {
      ...
      while(k<last)
        {
          ...
          AttariMethod_Vanilla_option(&op_ref,0.01,Levy);
...
void Constraints_ForLevyProcess(const PnlVect *x, PnlVect *res, void *data)
{
  ...
  for(j=0;j<op->nb_maturity;j++)
    {
      ...
      while(k<last)
        {
          op_ref=list_option_eqd_get_value(op,j,k);
          AttariMethod_Vanilla_option_LD(&op_ref,0.01,Levy);
          LET(op->price,k)=op_ref.price;
...
void Constraints_ForLevyDiffusion(const PnlVect *x, PnlVect *res, void *data)
{
