...
const double        GL5_wg[] = { 0.23692688505618908751,
  ...
  double    nig_generic_density(const double        x,
                                            const double        alpha,
                                            const double        beta,
                                            const double        gamma,
                                            const double        mu,
                                            const double        delta)
{
  ...
  return ( (delta * alpha * exp(delta * gamma + beta * (x-mu)) * bessel_K1(alpha * f_x)) / (C_PI * f_x) );
}
                                           ...
                                           const double     beta)
{
  ...
  if (y <= 0) return ( 0. );
  return ( C_SQRT1_2PI * (alpha / sqrt(beta)) * pow(y, -1.5) * exp(- z*z / (2. * beta * y)) );
}
                                    ...
                                    const double        delta)
{
  ...
  for (y = MINDOUBLE; y < 4.; y += h) {
    ...
    s1 += gaussian_cdf(z) * ig_generic_density(y+0.5*h, delta * gamma, gamma * gamma);
  }
  s1 *= h;
  s2 = 0;
  h = exp(-4.)/20.;
  for (t = MINDOUBLE; t < exp(-4.); t += h) {
    ...
    s2 += gaussian_cdf(z) * ig_generic_density(y, delta * gamma, gamma * gamma) * (1./(t+0.5*h));
  }
  s2 *= h;
