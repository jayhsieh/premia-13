...
double f1(double rho,double t1,double x1){
	double u=exp((t1+1)*0.5*log(1+x1*x1/(rho*rho*(t1-2))));
	return 1/(u);
}

double f2(double rho,double t2,double x2){
...
double fval1,fval2,h1,centre1,x1;

double a1=-6*(p->rho)*sqrt((p->t1-2)/p->t1);
double b1=6*(p->rho)*sqrt((p->t1-2)/p->t1);
double b2=6*sqrt(1-(p->rho)*(p->rho))*sqrt((p->t2-2)/p->t2);
double a2=-6*sqrt(1-(p->rho)*(p->rho))*sqrt((p->t2-2)/p->t2);
...
double coefs=gammas((p->t1+1)*0.5)*gammas((p->t2+1)*0.5)/((gammas(p->t1*0.5))*(gammas(p->t2*0.5))*pi*(p->rho)*sqrt((1-p->rho*p->rho)*(p->t1-2)*(p->t2-2)));
...
double double_t_cdf(copula *cop,double x){
int n=2*cop->size+100;
int i=0;
double result;
double_t_params *p;
...
double double_t_inv_cdf(const copula *cop,double x){
int n=2*cop->size+100;
int i,u,v;
double result;
double_t_params *p;
...
double_t_params *p;
p=cop->parameters;
return (gammas((p->t1+1)*0.5)/((gammas((p->t1)*0.5))*sqrt(3.14159265*(p->t1))*exp((((p->t1)+1)*0.5)*log(1+x*x/(p->t1)))));
