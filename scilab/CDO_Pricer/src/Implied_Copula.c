...
/**Dans cette partie nous définirons dans un premiers temps nos n lambda implicite selon les valeurs max qu'on fixera 
et dans un second temps nous définirons la matrice du gradient utile pour résoudre notre problème d'optimisation voir Papier Hull& White implied Copula**/
...
double          *lambdaimpl(const prod *produit,
                            ...
                            const int choix)
{
/** n définit le nombre d'intensité implicite que l'on veut avoir,ce nombre  dépend de l'utilisateur plus il est grand plus la calibration est meilleure **/
/**on définit l'intensité minimale et l'intensité maximale de défaut qu'on peut avoir grâce aux données du marché**/
int k,i,j;
...
/**On définit la somme de la valeur des contrats de nos six dérivés de crédit d'une part en considérant l'intensité minimale et d'une autre part en considérant l'intensité maximale**/
...
/** on retournera les lambda implicites tels que ces derniers couvrent uniformément [V1,V2] **/
...
/** Dans cette partie on cherche à résoudre le problème d'optimisation voir HUll & White Perfect Copula pour déterminer les probabilités associées à chaque intensité  on utilisera la méthode du gradient conjugué **/ 
int i,l;
int j=0;
double *P;
double *g;
